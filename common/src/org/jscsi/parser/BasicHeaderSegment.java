 ...
 * 
 * $Id: BasicHeaderSegment.java 2498 2007-03-05 12:32:43Z lemke $
 * 
 */
 ...
 * <code>BHS_FIXED_SIZE</code>. And these must the first bytes in a valid
 * iSCSI Protocol Data Unit (PDU).
 * org.jscsi.core.utils
 * @author Volker Wildi
 */
public final class BasicHeaderSegment {
  ...
  /**
   * The operation code indicates the type of iSCSI PDU the header encapsulates.
   * The operation codes are divided into two categories: <em>initiator</em>
   * opcodes and <em>target</em> opcodes. Initiator opcodes are in PDUs sent
   * by the initiator (request PDUs). Target opcodes are in PDUs sent by the
  ...
  /**
   * The initiator assigns a Task Tag to each iSCSI task it issues. While a task
   * exists, this tag MUST uniquely identify the task session-wide. SCSI may
   * also use the initiator task tag as part of the SCSI task identifier when
   * the timespan during which an iSCSI initiator task tag must be unique
  ...
  final int serialize(final ByteBuffer dst, final int offset)
    ...
    // has the destination array enough space to store this basic header segment
    dst.position(offset);
   ...
   * extraction this methods and setreturns the right message parser object for
  ...
  final void setOperationCode(final ProtocolDataUnit protocolDataUnit,
    ...
    parser = MessageParserFactory
        .getParser(protocolDataUnit, initOperationCode);
  }
