 ...
 * 
 * $Id: TaskManagementFunctionResponseParser.java 2500 2007-03-05 13:29:08Z lemke $
 * 
 */
 ...
 * <p>
 * <h4>Task Management Actions on Task Sets</h4>
 * The execution of ABORT TASK SET and CLEAR TASK SET Task Management function
 * requests consists of the following sequence of events in the specified order
...
public final class TaskManagementFunctionResponseParser extends
  ...
  public static enum ResponseCode {
    ...
    private ResponseCode(final byte newValue) {
      ...
      if (ResponseCode.mapping == null) {
        ResponseCode.mapping = new HashMap<Byte, ResponseCode>();
      }

      ResponseCode.mapping.put(newValue, this);
      value = newValue;
    }
   ...
   * </tr>
   * </table><br/><br/> All other values are reserved. <br/><br/> For a
   * discussion on usage of response codes <code>3</code> and <code>4</code>,
   * see Section 6.2.2 Allegiance Reassignment.<br/><br/> For the TARGET COLD
   * RESET and TARGET WARM RESET functions, the target cancels all pending
   * operations across all Logical Units known to the issuing initiator. For the
   * TARGET COLD RESET function, the target MUST then close all of its TCP
   * connections to all initiators (terminates all sessions).<br/><br/> The
   * mapping of the response code into a SCSI service response code value, if
   * needed, is outside the scope of this document. However, in symbolic terms
   * Response values 0 and 1 map to the SCSI service response of FUNCTION
   * COMPLETE. All other Response values map to the SCSI service response of
   * FUNCTION REJECTED. If a Task Management function response PDU does not
   * arrive before the session is terminated, the SCSI service response is
   * SERVICE DELIVERY OR TARGET FAILURE.<br/><br/> The response to ABORT TASK
   * SET and CLEAR TASK SET MUST only be issued by the target after all of the
   * commands affected have been received by the target, the corresponding task
   * management functions have been executed by the SCSI target, and the
   * delivery of all responses delivered until the task management function
   * completion have been confirmed (acknowledged through ExpStatSN) by the
   * initiator on all connections of this session. For the exact timeline of
   * events, refer to Section 10.6.2 Task Management Actions on Task Sets.<br/><br/>
