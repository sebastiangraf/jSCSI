...
public class SCSICommandParser extends InitiatorMessageParser {
  ...
  public enum TaskAttributes {
    ...
    /**
     * Untagged queuing allows a target to accept a command from an initiator
     * for a logical unit or target routine while I/O processes from other
     * initiators are being executed. Only one command for each I_T_x nexus
     * shall be accepted at a time.
     */
    UNTAGGED((byte) 0),
    /**
   ...
   * the Bidirectional Read Expected Data Transfer Length field correspond to
   * the SAM2 byte count.
   * <p>
   * If the Expected Data Transfer Length for a write and the length of the
   * immediate data part that follows the command (if any) are the same, then no
   * more data PDUs are expected to follow. In this case, the F bit MUST be set
   * to <code>1</code>.
   * <p>
