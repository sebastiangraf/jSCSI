 ...
 * LoginRequestParser.java 2500 2007-03-05 13:29:08Z lemke $
 */
 ...
 * <p>
 * If the <code>T</code> bit is set to <code>1</code> and <code>NSG</code> is
 * <code>FullFeaturePhase</code>, then this also indicates that the initiator is
 * ready for the Final Login Response (see Chapter 5).
 * <p>
 * <h4>T (Transit) Bit</h4> If set to <code>1</code>, indicates that the
 * initiator is ready to transit to the next stage.
 * <p>
 * If the T bit is set to <code>1</code> and NSG is FullFeaturePhase, then this
 * also indicates that the initiator is ready for the Final Login Response (see
 * Chapter 5).
 * <p>
 ...
 * CmdSN <code>123</code> and the first non-immediate command in
 ...
 * that PDU carries <code>CmdSN=500</code>. Subsequent Login Requests that are
 ...
 * <p>
 * <h4>ExpStatSN</h4> For the first Login Request on a connection this is
 * ExpStatSN for the old connection and this field is only valid if the Login
 * Request restarts a connection (see Section 5.3.4 Connection Reinstatement).
 * For subsequent Login Requests it is used to acknowledge the Login Responses
 * with their increasing StatSN values.
 * <p>
 * <h4>Login Parameters (Data Segment)</h4> The initiator MUST provide some
 * basic parameters in order to enable the target to determine if the initiator
 * may use the target’s resources and the initial text parameters for the
 * security exchange. All the rules specified in Section 10.10.5 Text for text
 * requests also hold for Login Requests. Keys and their explanations are listed
 ...
 * Chapter 11 only need to be supported when the function to which they refer is
