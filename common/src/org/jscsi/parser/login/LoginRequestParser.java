 ...
 * 
 * $Id: LoginRequestParser.java 2500 2007-03-05 13:29:08Z lemke $
 * 
 */
 ...
 * <p>
 * This class parses a Login Request message defined in the iSCSI Standard
 * (RFC3720).
 * <p>
 * After establishing a TCP connection between an initiator and a target, the
 * initiator MUST start a Login Phase to gain further access to the target’s
 * resources.
 * <p>
 ...
 * number of the current draft is <code>0x00</code>. As such, all devices
 * MUST carry version <code>0x00</code> for both Version-min and Version-max.
 * <p>
 ...
 * <p>
 * If the <code>T</code> bit is set to <code>1</code> and <code>NSG</code>
 * is <code>FullFeaturePhase</code>, then this also indicates that the
 * initiator is ready for the Final Login Response (see Chapter 5).
 * <p>
 * <h4>T (Transit) Bit</h4>
 * If set to <code>1</code>, indicates that the initiator is ready to transit
 * to the next stage.
 * <p>
 * If the T bit is set to <code>1</code> and NSG is FullFeaturePhase, then
 * this also indicates that the initiator is ready for the Final Login Response
 * (see Chapter 5).
 * <p>
 * <h4>CmdSN</h4>
 ...
 * sequence number in the command stream if the login is for a new connection in
 * an existing session.<br/> Examples:<br/>
 * <ul>
 * <li>Login on a leading connection - if the leading login carries the CmdSN
 * <code>123</code>, all other Login Requests in the same Login Phase carry
 * the CmdSN <code>123</code> and the first non-immediate command in
 * FullFeaturePhase also carries the CmdSN <code>123</code>. </li>
 * <li>Login on other than a leading connection - if the current CmdSN at the
 * time the first login on the connection is issued is <code>500</code>, then
 * that PDU carries <code>CmdSN=500</code>. Subsequent Login Requests that
 * are needed to complete this Login Phase may carry a CmdSN higher than
 * <code>500</code> if non-immediate requests that were issued on other
 * connections in the same session advance CmdSN.</li>
 * </ul>
 ...
 * <p>
 * <h4>ExpStatSN</h4>
 * For the first Login Request on a connection this is ExpStatSN for the old
 * connection and this field is only valid if the Login Request restarts a
 * connection (see Section 5.3.4 Connection Reinstatement). For subsequent Login
 * Requests it is used to acknowledge the Login Responses with their increasing
 * StatSN values.
 * <p>
 * <h4>Login Parameters (Data Segment)</h4>
 * The initiator MUST provide some basic parameters in order to enable the
 * target to determine if the initiator may use the target’s resources and the
 * initial text parameters for the security exchange. All the rules specified in
 * Section 10.10.5 Text for text requests also hold for Login Requests. Keys and
 * their explanations are listed in Chapter 11 (security negotiation keys) and
 * Chapter 12 (operational parameter negotiation keys). All keys in Chapter 12,
 * except for the X extension formats, MUST be supported by iSCSI initiators and
 * targets. Keys in Chapter 11 only need to be supported when the function to
 * which they refer is mandatory to implement.
 * <p>
 * 
 * @author Volker Wildi
 */
public final class LoginRequestParser extends InitiatorMessageParser {
   ...
   * ends a set of key=value pairs. A Login Request with the <code>C</code>
   * bit set to <code>1</code> MUST have the <code>T</code> bit set to
   * <code>0</code>.
   * 
