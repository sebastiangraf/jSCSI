 ...
 * ProtocolDataUnit.java 2498 2007-03-05 12:32:43Z lemke $
 */
...
public final class ProtocolDataUnit {
   ...
   * <code>DataSegmentLength</code>. The Data Segment is also padded to multiple
   * of a <code>4</code> byte words.
   */
   ...
   * <p>
   * <b>The existence and type of digests are negotiated during the Login Phase.
   * </b>
   * <p>
   * The separation of the header and data digests is useful in iSCSI routing
   * applications, in which only the header changes when a message is forwarded.
   * In this case, only the header digest should be recalculated.
   * <p>
   ...
   *          The instance of the digest to use for the Data Segment protection.
   */
  ...
  /**
   * Reads from the given <code>SocketChannel</code> all the neccassary bytes to
   * fill this PDU.
   * 
   * @param sChannel
  ...
  public final int read(final SocketChannel sChannel)
    ...
    while (len < BasicHeaderSegment.BHS_FIXED_SIZE) {
      ...
      if (lens == -1) {
        //The Channel was closed at the Target (e.g. the Target does not support Multiple Connections)
