 ...
 * AbstractDataSegment.java 2498 2007-03-05 12:32:43Z lemke $
 */
...
public abstract class AbstractDataSegment implements IDataSegment {
  ...
  /**
   * Constructor to create a new, empty <code>DataSegment</code> object with the
   * given chunk size.
   * 
  ...
  public int serialize(final ByteBuffer dst, final int off) {
    ...
    if (dst.remaining() % Constants.BYTES_PER_INT != 0) {
      throw new IllegalArgumentException(
          "The buffer length must be a multiple of " + Constants.BYTES_PER_INT
              + ".");
    }
   ...
   * needed, when the <code>neededLength</code> is greater than the capacity of
   * allocated data buffer. The flag <code>copyData</code> indicates, if the old
   * buffer has to be copied into the new bigger data buffer.
   * 
   * @param additionalLength
   *          The length, which is now needed to store all informations in the
   *          data buffer.
   * @param copyData
  ...
  protected void resizeBuffer(final int additionalLength, final boolean copyData) {
    ...
    if (length + additionalLength > dataBuffer.capacity()) {
      ...
      final ByteBuffer newBuffer = ByteBuffer.allocate(getTotalLength(length
          + additionalLength));
   ...
   * @param anObject
   *          Another object.
   * @return <code>true</code>, if the <code>anObject</code> and this object are
  ...
  /**
   * Iterator through the chunks of a <code>DataSegment</code>. <p/><b>This
   * class is not resistant against changes to the <code>dataBuffer</code> .</b>
   */
  private final class DataSegmentIterator implements IDataSegmentIterator {
    ...
    /**
     * <h1>DataSegmentChunk</h1> <p/> This is a chunk of a data segment.
     * 
     * @author Volker Wildi
     */
    private final class DataSegmentChunk implements IDataSegmentChunk {
       ...
       * instance with a given length and a total length of this data segment
       * chunk.
       * 
       * @param initLength
