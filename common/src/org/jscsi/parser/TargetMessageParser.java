 ...
 * TargetMessageParser.java 2498 2007-03-05 12:32:43Z lemke $
 */
...
public abstract class TargetMessageParser extends AbstractMessageParser {
   ...
   * of the <code>Expected Status Sequence Number</code>.
   * 
   ...
   * <code>ExpCmdSN</code> is a Sequence Number that the target iSCSI returns to
   * the initiator to acknowledge command reception. It is used to update a
   ...
   * indicate the maximum <code>CmdSN</code> the initiator can send. It is used
   * to update a local variable with the same name. If <code>MaxCmdSN</code> is
   * equal to <code>ExpCmdSN-1</code>, this indicates to the initiator that the
   * target cannot receive any additional commands. When <code>MaxCmdSN</code>
   * changes at the target while the target has no pending PDUs to convey this
   * information to the initiator, it MUST generate a NOP-IN to carry the new
   * <code>MaxCmdSN</code>.
   * 
