 ...
 * 
 * $Id: DataInParser.java 2498 2007-03-05 12:32:43Z lemke $
 * 
 */
...
public class DataInParser extends TargetMessageParser {
   ...
   * reserved. The Target Transfer Tag and LUN are copied by the initiator into
   * the SNACK of type DataACK that it issues as a result of receiving a SCSI
   * Data-In PDU with the A bit set to <code>1</code>.<br/> The Target
   * Transfer Tag values are not specified by this protocol except that the
   * value <code>0xffffffff</code> is reserved and means that the Target
   * Transfer Tag is not supplied. If the Target Transfer Tag is provided, then
   * the LUN field MUST hold a valid value and be consistent with whatever was
   * specified with the command; otherwise, the LUN field is reserved.
   * 
   ...
   * use the A bit moderately; it MAY only set the A bit to <code>1</code>
   ...
   * session with ErrorRecoveryLevel greater than <code>0</code>, if there
   * are no holes in the read data until that Data-In PDU, the initiator MUST
   * issue a SNACK of type DataACK except when it is able to acknowledge the
   * status for the task immediately via ExpStatSN on other outbound PDUs if the
   * status for the task is also received. In the latter case (acknowledgement
   * through ExpStatSN), sending a SNACK of type DataACK in response to the A
   * bit is OPTIONAL, but if it is done, it must not be sent after the status
   ...
   * 
   * @return <code>True</code>, if the ResidualOverflow-Flag of this object
   ...
   * bit is set to <code>1</code>, the <code>F bit</code> MUST also be set
   * to <code>1</code>.<br/> The fields StatSN, Status, and Residual Count
   * only have meaningful content if the S bit is set to <code>1</code> and
   * their values are defined in Section 10.4 SCSI Response.
   * 
   * @return <code>True</code>, if the Status-Flag of this object is set.
