 ...
 * DataInParser.java 2498 2007-03-05 12:32:43Z lemke $
 */
...
public class DataInParser extends TargetMessageParser {
   ...
   * Task Tag. <br/> R2T and Data-In PDUs, in the context of bidirectional
   * commands, share the numbering sequence (see Section 3.2.2.3 Data
   * Sequencing). <br/> For output (write) data PDUs, the DataSN is the Data-Out
   * PDU number within the current output sequence. The current output sequence
   * is either identified by the Initiator Task Tag (for unsolicited data) or is
   ...
   * a replica of the Target Transfer Tag provided with the R2T. <br/> On
   * incoming data, the Target Transfer Tag and LUN MUST be provided by the
   * target if the A bit is set to <code>1</code>; otherwise they are reserved.
   * The Target Transfer Tag and LUN are copied by the initiator into the SNACK
   * of type DataACK that it issues as a result of receiving a SCSI Data-In PDU
   * with the A bit set to <code>1</code>.<br/> The Target Transfer Tag values
   * are not specified by this protocol except that the value
   * <code>0xffffffff</code> is reserved and means that the Target Transfer Tag
   * is not supplied. If the Target Transfer Tag is provided, then the LUN field
   * MUST hold a valid value and be consistent with whatever was specified with
   * the command; otherwise, the LUN field is reserved.
   * 
   ...
   * acknowledgement from the initiator for the data received. The target should
   * use the A bit moderately; it MAY only set the A bit to <code>1</code> once
   * every MaxBurstLength bytes, or on the last Data-In PDU that concludes the
   * entire requested read data transfer for the task from the target’s
   * perspective, and it MUST NOT do so more frequently. The target MUST NOT set
   * to <code>1</code> the <code>A</code> bit for sessions with
   * ErrorRecoveryLevel=<code>0</code>. The initiator MUST ignore the A bit
   * <br/>
   * <p>
   ...
   * no holes in the read data until that Data-In PDU, the initiator MUST issue
   * a SNACK of type DataACK except when it is able to acknowledge the status
   * for the task immediately via ExpStatSN on other outbound PDUs if the status
   * for the task is also received. In the latter case (acknowledgement through
   * ExpStatSN), sending a SNACK of type DataACK in response to the A bit is
   * OPTIONAL, but if it is done, it must not be sent after the status
   * acknowledgement through ExpStatSN. If the initiator has detected holes in
   * the read data prior to that Data-In PDU, it MUST postpone issuing the SNACK
   * of type DataACK until the holes are filled. An initiator also MUST NOT
   * acknowledge the status for the task before those holes are filled. A status
   * acknowledgement for a task that generated the Data-In PDUs is considered by
   * the target as an implicit acknowledgement of the Data-In PDUs if such an
  ...
  /**
   * In this case, the Bidirectional Read Residual Count indicates the number of
   ...
   * 
   * @return <code>True</code>, if the ReadResidualOverflow-Flag of this object
   ...
   * 
   * @return <code>True</code>, if the ReadResidualUnderflow-Flag of this object
   *         is set. Else <code>false</code>.
   */
   ...
   * not sufficient. For a bidirectional operation, the Residual Count contains
   ...
   * transferred. For a bidirectional operation, the Residual Count contains the
   * residual for the write operation.
   * 
   * @return <code>True</code>, if the ResidualUnderflow-Flag of this object is
   *         set. Else <code>false</code>.
   */
   ...
   * not received a Data PDU with the final bit Set), the target MUST wait until
   * it receives a Data PDU with the F bit set in the last expected sequence
  ...
  /**
   * Set this to indicate that the Command Status field contains status. If this
   * bit is set to <code>1</code>, the <code>F bit</code> MUST also be set to
   * <code>1</code>.<br/> The fields StatSN, Status, and Residual Count only
   * have meaningful content if the S bit is set to <code>1</code> and their
   * values are defined in Section 10.4 SCSI Response.
   * 
