...
public class InitiatorBench {
	...
	/**
	 * Benchmark to measure the time needed to read 10 blocks in sequential
	 * order.
	 * 
	 * @throws Exception
	 *             if any error occurs.
	...
	@SkipBench
	private final void readSequentialBlocks(final ByteBuffer dst,
			final int blocks) throws Exception {

		int logicalBlockAddress = getLastBlockAddress(blocks);
		for (int i = 0; i < blocks; i++) {
			initiator.read(this, TARGET_NAME, dst, logicalBlockAddress,
					BLOCK_SIZE);
			logicalBlockAddress++;
		}
	}
	...
	private final void writeSequentialBlocks(final ByteBuffer src,
		...
		int logicalBlockAddress = getLastBlockAddress(blocks);
		for (int i = 0; i < blocks; i++) {
			initiator.write(this, TARGET_NAME, src, logicalBlockAddress,
					BLOCK_SIZE);
			logicalBlockAddress++;
		}
	}
