...
public final class Initiator {
    ...
    public final void createSession(final String targetName) throws Exception {
        ...
        createSession(configuration.getTargetAddress(targetName), configuration.getTargetPort(targetName), targetName);
    }
     ...
     *             if any error occurs.
     */
    public final void createSession(final InetAddress targetAddress, final int port, final String targetName) throws Exception {

        LOGGER.info("Created the session with iSCSI Target '" + targetName + "' at " + targetAddress + " on port " + port + ".");

        final Session session = new Session(configuration, targetName, targetAddress, port, 1);
     ...
     * the logical block address and request <code>transferLength</code>
     * blocks.
     * 
     * @param caller
     ...
     *             if any error occurs.
     */
    public final void read(final Object caller, final String targetName, final ByteBuffer dst, final int logicalBlockAddress, final long transferLength)
            throws Exception {

        getSession(targetName).read(caller, dst, logicalBlockAddress, transferLength);
    }

    /**
     * Invokes a write operation for the session <code>targetName</code> and
     * transmits the bytes in the buffer <code>dst</code>. Start writing at
     * the logical block address and transmit <code>transferLength</code>
     * blocks.
     * 
     * @param caller
     *            The pointer to the calling instance of this method.
     ...
     *             if any error occurs.
     */
    public final void write(final Object caller, final String targetName, final ByteBuffer src, final int logicalBlockAddress, final long transferLength)
            throws Exception {

        getSession(targetName).write(caller, src, logicalBlockAddress, transferLength);
    }
