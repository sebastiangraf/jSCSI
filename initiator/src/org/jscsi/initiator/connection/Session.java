...
import org.jscsi.initiator.taskbalancer.ITaskBalancer;
...
/**
 * <h1>AbsSession</h1> <p/> A session or Initiator Target Nexus is a directed
 * communication from an iSCSI Initiator to an iSCSI Target. Each session can
 * contain several connections. This allows a better usage of bandwidth and
 * decreases latency times. The Abstract Class is used to implement serveral
 * single- and multithreaded variants of Sessions
 * 
...
public final class Session {
  ...
  /**
   * This instance contains the informations about the capacity of the connected
   * target.
   */
   ...
   * appropriate command. And the target uses this tag to correlate the data to
   * the appropriate command that it received earlier.
   */
   ...
   * connections.
   */
  protected final ITaskBalancer taskBalancer;
   ...
   * maximum number of allowed connections to a given iSCSI Target. This is the
   * abstract definition for Session implementations
   * 
   * @param linkFactory
   ...
   * @param inetAddress
   *          The <code>InetSocketAddress</code> of the leading connection of
   *          this session.
   * @param initExecutor
  ...
  public Session(final LinkFactory linkFactory,
      ...
      final InetSocketAddress inetAddress, final ExecutorService initExecutor)
  throws Exception {
    ...
    /*
     * We have to check whether the MaxConnection setting in our Configuration
     * is correct. There might be a wrong setting for a target e.g. the target
     * only supports one connection but we think it can handle two.
     */
    maxConnections = Integer.parseInt(configuration.getSessionSetting(targetName,
    ...
    // TODO Do something more intelligent here. Always adding the maximum isn't
    // always a good idea
  ...
  /**
   * Sets the Target Session Identifying Handle (TSIH) to the given value. This
   * TSIH is specified at the Login Phase by the target in a new session. So, it
   ...
   *         accept more incoming PDUs. Else <code>false</code> and hold out for
   *         sending.
   */
   ...
   *           if any error occurs.
   */
  public final void addConnections(final int max)
  throws Exception {
  ...
  /**
   * Adds a new connection to this session with the next free connection ID (if
   * the maximum number is not reached).
   * 
   * @return The connection ID of the newly created connection.
   * @throws Exception
   *           if any error occurs.
   */
  protected final short addNewConnection()
  throws Exception {

    if (connections.size() < maxConnections) {

      final Connection connection = factory.getConnection(this, configuration,
          inetSocketAddress, nextFreeConnectionID);
   ...
   * 
   * @param max 
   *          The maximum number of concurrent <code>Connections</code> to a target.
   */
  ...
  public final Future<Void> write(final Object caller, final ByteBuffer src,
    ...
    return executeTask(new WriteTask(caller, this, src, logicalBlockAddress,
        transferLength));
  }
  ...
  /**
   * This methods appends the given task to the end of the taskQueue and set the
   * calling thread is sleep state.
   * 
   * @param task
   ...
   */
  public final void restartTask(final ITask task)
  throws ExecutionException {
