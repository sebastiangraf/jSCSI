...
public final class Session implements Runnable {
   ...
   *           if any error occurs.
   */
  public Session(final Configuration initConfiguration,
      final String initTargetName, final InetAddress inetAddress,
      final int port, final int initMaxConnections) throws Exception {

    if (initMaxConnections > Short.MAX_VALUE) {
      throw new IllegalArgumentException(
          "The Maximum number cannot be greater than " + Short.MAX_VALUE);
    }
    ...
    new Thread(this, "SessionTaskThread for iSCSI Target " + targetName)
        .start();
   ...
   *           if any error occurs.
   */
  private final short addNewConnection(final InetAddress inetAddress,
    ...
    final Connection connection = new Connection(this, configuration,
        inetAddress, port, nextFreeConnectionID);
    connection.execute();
   ...
   */
  final void setMaximumCommandSequenceNumber(
   ...
   *           if any error occurs.
   */
  public final void read(final Object caller, final ByteBuffer dst,
      final int logicalBlockAddress, final long transferLength)
      throws Exception {

    appendAndSleep(new ReadTask(caller, this, dst, logicalBlockAddress,
        transferLength));
  }
   ...
   *           if any error occurs.
   */
  public final void write(final Object caller, final ByteBuffer src,
      final int logicalBlockAddress, final long transferLength)
      throws Exception {

    appendAndSleep(new WriteTask(caller, this, src, logicalBlockAddress,
        transferLength));
  }
   ...
   *           exception is thrown.
   */
  private final void appendAndSleep(final ITask task)
      throws InterruptedException {
    ...
    synchronized (task) {
      ...
      if (LOGGER.isInfoEnabled()) {
        LOGGER.info("Added a " + task + " to the task queue of the session "
            + targetName);
      }
  ...
  public final void run() {
    ...
    while (!stop) {
      ...
      if (head != null) {
        try {
          ...
          if (LOGGER.isInfoEnabled()) {
            LOGGER.info("Polled a " + head.getClass().getSimpleName()
                + " form the task queue of the session " + targetName);
          }
    ...
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug("SessionTaskThread for session " + targetName
          + " is stopped.");
    }
  }
  ...
  private interface ITask {
    ...
    /**
     * This method is call, when this <code>ITask</code> instance is polled
     * from the head of the <code>taskQueue</code> to start a task.
     * 
     * @throws Exception
     *           if any error occurs.
     */
