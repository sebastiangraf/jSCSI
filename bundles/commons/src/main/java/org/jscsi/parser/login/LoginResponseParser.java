 ...
 * LoginResponseParser.java 2500 2007-03-05 13:29:08Z lemke $
 */
 ...
 * <p>
 * <h4>T (Transit) bit</h4> The <code>T</code> bit is set to <code>1</code> as
 * an indicator of the end of the stage. If the <code>T</code> bit is set to
 * <code>1</code> and NSG is FullFeaturePhase, then this is also the Final Login
 * Response (see Chapter 5). A <code>T</code> bit of <code>0</code> indicates a
 ...
 * initiator within the same stage.
 * <p>
 * <h4>StatSN</h4> For the first Login Response (the response to the first Login
 * Request), this is the starting status Sequence Number for the connection. The
 * next response of any kind, including the next Login Response, if any, in the
 * same Login Phase, will carry this <code>number + 1</code>. This field is only
 * valid if the Status-Class is <code>0</code>.
 * <p>
 * <h4>Login Parameters</h4> The target MUST provide some basic parameters in
 * order to enable the initiator to determine if it is connected to the correct
 * port and the initial text parameters for the security exchange.
 * <p>
...
public final class LoginResponseParser extends TargetMessageParser {
   ...
   * <ul>
   * <li>Status-Class</li> <li>Status-Detail</li>
   * </ul>
   * <p>
