 ...
 * TextResponseParser.java 2500 2007-03-05 13:29:08Z lemke $
 */
...
public final class TextResponseParser extends TargetMessageParser {
   ...
   * subsequent Text Responses); otherwise, it indicates that this Text Response
   * ends a set of key=value pairs. A Text Response with the <code>C</code> bit
   * set to <code>1</code> MUST have the <code>F</code> bit set to
   * <code>0</code>.
   * 
  ...
  /**
   * When a target has more work to do (e.g., cannot transfer all the remaining
   * text data in a single Text Response or has to continue the negotiation) and
   * has enough resources to proceed, it MUST set the Target Transfer Tag to a
   ...
   * that it wants the rest of the data. When the target receives a Text Request
   * with the Target Transfer Tag set to the reserved value of
   * <code>0xffffffff</code>, it resets its internal information (resets state)
   * associated with the given Initiator Task Tag (restarts the negotiation).
   * When a target cannot finish the operation in a single Text Response, and
   * does not have enough resources to continue, it rejects the Text Request
