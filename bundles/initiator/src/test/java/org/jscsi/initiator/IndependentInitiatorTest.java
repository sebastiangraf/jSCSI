...
public class IndependentInitiatorTest {
	...
	public static class CallableStart {
		 ...
		 */
		public static void start() throws SAXException,
				ParserConfigurationException, IOException,
				ConfigurationException {
			if (isWindows()) {
				targetConfigurationFile = new File(CONFIG_DIR,
						"jscsi-target-windows.xml");
			} else {
				targetConfigurationFile = new File(CONFIG_DIR,
						"jscsi-target-linux.xml");
			}

			org.jscsi.target.Configuration targetConfiguration = org.jscsi.target.Configuration
					.create(new File(CONFIG_DIR, "jscsi-target.xsd"),
	...
	public static final void initialize() throws Exception {
		...
		configuration = Configuration.create(new File(CONFIG_DIR, "jscsi.xsd"),
				new File(CONFIG_DIR, "jscsi.xml"));
		...
		session = factory.getSession(configuration, TARGET_DRIVE_NAME,
				configuration.getTargetAddress(TARGET_DRIVE_NAME));
	}
	...
	public final void testReadWriteEquality() throws Exception {
		Future<Void> returnVal1 = session.write(writeBuffer,
				LOGICAL_BLOCK_ADDRESS, writeBuffer.remaining());
		returnVal1.get();

		assertTrue(!writeBuffer.equals(readBuffer));

		Future<Void> returnVal2 = session.read(readBuffer,
	...
	public final void testMultipleReadWrite() {
		...
		for (int i = 0; i < writeBuffers.length; i++) {
			...
			try {
				returnVal1 = session.write(writeBuffers[i],
						LOGICAL_BLOCK_ADDRESS, writeBuffers[i].remaining());
		...
		for (int i = 0; i < writeBuffers.length; i++) {
			Future<Void> returnVal1;
			try {
				returnVal1 = session.read(readBuffers[i],
						LOGICAL_BLOCK_ADDRESS, readBuffers[i].remaining());
		...
		for (int i = 0; i < writeBuffers.length; i++) {
			assertTrue(writeBuffers[i].equals(readBuffers[writeBuffers.length
					- i - 1]));
