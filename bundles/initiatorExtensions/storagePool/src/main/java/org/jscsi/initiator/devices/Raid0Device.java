 ...
 * Raid1Device.java 2642 2007-04-10 09:46:49Z lemke $
 */
...
public class Raid0Device implements Device {
  ...
  public void read(final long address, final byte[] data) throws Exception {
    ...
    if (data.length % extendSize != 0) {
      throw new IllegalArgumentException(
          "Number of bytes is not a multiple of the extend size (" + extendSize
              + ")!");
    }
  ...
  public void write(final long address, final byte[] data) throws Exception {
    ...
    for (int i = 0; i < fragments; i++) {
      System.arraycopy(data, i * extendSize,
          deviceData.get(i % devices.length), i / devices.length * extendSize,
          extendSize);
    }
    for (int i = 0; i < parts; i++) {
