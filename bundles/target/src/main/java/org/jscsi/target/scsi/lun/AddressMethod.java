...
public enum AddressMethod {
     ...
     * level.
     * <p>
     * Extended logical unit addressing builds on the formats defined for
     * dependent logical units (see 4.6.19.4) but may be used by SCSI devices
     * having single level logical unit structure. In dependent logical unit
     * addressing, the logical unit information at each level fits in exactly
     * two bytes. Extended logical unit addresses have sizes of two bytes, four
     * bytes, six bytes, or eight bytes.
     */
    ...
    /**
     * Returns the {@link AddressMethod} with a matching {@link #value} or
     * <code>null</code> if no such {@link AddressMethod} exists.
     * 
     * @param value
     *            the value from a serialized ADDRESS METHOD field
     * @return the {@link AddressMethod} with a matching {@link #value} or or
     *         <code>null</code>
     */
