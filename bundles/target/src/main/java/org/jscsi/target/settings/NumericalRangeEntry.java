 ...
 * require the iSCSI initiator to offer an interval from which the target
 * must choose the final value. Thereby the <i>key=value</i> format differs
 * from that required by the {@link NumericalEntry} class, for which <i>value</i>
 * is a single integer. The <i>value</i> part expected by this class looks like
 ...
 * <code>OFMarker=No</code>), the correct response value to <code>IFMarkInt</code>
 * and <code>OFMarkInt</code> <i>keys</i> must be <code>Irrelevant</code>. This
 * behavior is part of the {@link Entry#negotiate(org.jscsi.parser.login.LoginStage,
 * boolean, boolean, String, String, java.util.Collection)} method, which will
 * lead to correct responses without having to check for additional constraints
 * (the presence and <i>value</i> of <code>IFMarker</code> and <code>OFMarker</code>
 * <i>key=value</i> pairs.
...
public final class NumericalRangeEntry extends Entry {
	...
	/**
	 * A {@link NumericalValueRange} specifying the boundaries into which
	 * any RFC-conform <i>value</i> range proposed by the iSCSI initiator
	 * must fall.
	 */
	 ...
	 * @param protocolValueRange specifies the boundaries into which
	 * any RFC-conform <i>value</i> range proposed by the iSCSI initiator
	 * must fall
	 * @param defaultValue the default value or <code>null</code>
	 */
	public NumericalRangeEntry(final KeySet keySet,
			final Use use,
			final NegotiationStatus negotiationStatus,
			final int negotiationValue,
			final NumericalValueRange protocolValueRange,
			final Object defaultValue) {
		super(keySet, NegotiationType.NEGOTIATED, use,
				negotiationStatus, defaultValue);
	...
	protected Object parseOffer(String values) {
		...
		if (range == null && Target.config.getAllowSloppyNegotiation()) {
			 ...
			 */
			final SingleNumericalValue singleValue =
				SingleNumericalValue.parseSingleNumericValue(values);
			if (singleValue != null) {
	...
	@Override
	public Entry copy() {
		final NumericalRangeEntry e = new NumericalRangeEntry(keySet,
				use, negotiationStatus, negotiationValue,
				protocolValueRange, (Integer)value);
