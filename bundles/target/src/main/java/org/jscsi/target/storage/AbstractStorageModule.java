...
/**
 * This is an abstract super class offering methods for storage and
 * retrieval of data, as well as emulating some properties of block storage devices.
 * <p>
 * All index and length parameters used by the read and write methods are
 * referring to bytes, unlike the values sent in {@link CommandDescriptorBlock}s,
 * which are based on the value reported by {@link #getBlockSizeInBytes()}.
...
public abstract class AbstractStorageModule {
	...
	/**
	 * The abstract constructor that makes sure that the
	 * {@link #sizeInBlocks} variable is initialized.
	 * @param sizeInBlocks the size of the medium in blocks
	 */
	...
	/**
	 * Returns the storage space size in bytes divided by {@link #getBlockSizeInBytes()}
	 * (rounded down).
	 ...
	 * @param bytes the array into which the data will be copied
	 * @param bytesOffset the position of the first byte in <code>bytes</code>,
	 * which will be filled with data from storage
	 ...
	 * @param bytes the array into which the data will be copied
	 * @param storageIndex he position of the first byte to be copied 
	 */
	public final void read(byte[] bytes, long storageIndex)
	throws IOException  {
	 ...
	 * @param bytesOffset offset of the first byte to be stored
	 * @param length the number of bytes to be copied
	 * @param storageIndex byte offset in the storage area
	 * @throws IOException
	 */
	public abstract void write(byte[] bytes, int bytesOffset,
			int length, long storageIndex) throws IOException ;
	 ...
	 * @throws IOException
	 */
	public final void write(byte[] bytes, long storageIndex)
	throws IOException {
		write(bytes, 0, bytes.length, storageIndex);
	}
	
	/**
	 * This method returns a human-friendly {@link String} representation
	 * of the medium's size.
	 * <p>
	 * Subclasses that do not enforce <code>MEDIUM SIZE %
	 * {@link #VIRTUAL_BLOCK_SIZE} == 0</code> should overwrite this method
	 * to report unused bytes, if any.
	...
	/**
	 * This method can be used for checking if a (series of) I/O operations
	 * will result in an {@link IOException} due to trying to access blocks
	 * outside the medium's boundaries.
	 * <p>
	 * The SCSI standard requires checking for these boundary violations
	 * right after receiving a read or write command, so that an appropriate
	 * error message can be returned to the initiator. Therefore this method
	 ...
	 *  <tr>
	 *   <td>2</td>
	 *   <td>the interval [<i>logicalBlockAddress</i>,
	 *   <i>logicalBlockAddress</i> + <i>transferLengthInBlocks</i>]<br/>
	 *   lies outside of the interval, or <i>transferLengthInBlocks</i>
	 *   is negative</td>
	 *  </tr>
	 * </table>
	 * <p>
	 * Note that the parameters of this method are referring to blocks, not
	 * to byte indices.
	 * @param logicalBlockAddress the index of the first block of data to be
	 * read or written
	 * @param transferLengthInBlocks the total number of consecutive blocks
	 ...
	 * @param sizeInBytes a value specifying a certain number of bytes
	...
	protected static final String toHumanFriendlySize(final long sizeInBytes) {
		...
		values[values.length - 1] = size;//the value for the biggest unit may be >= factor
