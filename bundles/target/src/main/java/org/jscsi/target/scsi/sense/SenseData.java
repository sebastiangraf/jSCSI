...
/**
 * Sense data shall be returned in the same I_T_L_Q nexus transaction
 * as a CHECK CONDITION status and as parameter data in response to
 * the REQUEST SENSE command.
 * <p>
 * Sense data returned in the same I_T_L_Q nexus transaction as a
 * CHECK CONDITION status shall be either fixed or descriptor format
 * sense data format based on the value of the D_SENSE bit in the
 * Control mode page.
 * <p>
 * The REQUEST SENSE command may be used to request either the fixed
 * format sense data or the descriptor format sense data.
 * @author Andreas Ergenzinger
 */
public abstract class SenseData implements ISerializable {
	
	/**
	 * The first byte of all sense data contains the RESPONSE CODE
	 * field that indicates the error type and format of the sense
	 * data.
	 * @see #errorType
	 * @see #senseDataFormat
	 */
	...
	/**
	 * The {@link #senseKey} and {@link #additionalSenseCodeAndQualifier} fields provide a
	 * hierarchy of information. The hierarchy provides a top-down approach for an application
	 * client to determine information relating to the error and exception conditions.
	 */
	 ...
	 * @param senseKey describes the general category of the error requiring
	 * the sending of sense data
	 * @param additionalSenseCodeAndQualifier a more specific description of
	 * the error
	 */
	public SenseData(
	...
	/**
	 * Returns the proper response code for the given error type and sense
	 * data format.
	...
	public static final int getReponseCodeFor(final ErrorType errorType,
		...
		/*
		 * Response codes 0x74 to 0x7E are reserved.
		 * Response code 0x7f is vendor specific.
		 */
	}
}
