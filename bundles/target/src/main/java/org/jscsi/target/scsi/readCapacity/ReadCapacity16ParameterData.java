 ...
 * <p>
 * Only the fields common to all {@link ReadCapacityParameterData} can be
 * set in the constructor. All other fields and flags are <code>zero</code>.
 * This means that the initiator is told that the device does not support
 * protection information (protection type 0), maps each logical block
 * directly to one physical block, beginning with the first one.
