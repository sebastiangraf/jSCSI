...
public final class TargetFullFeaturePhase extends TargetPhase {
	...
	/**
	 * While this variable is <code>true</code> the phase is still
	 * running, either executing a specific stage or waiting for the
	 * next one to begin.
	 */
			...
			switch (bhs.getOpCode()) {
				...
				case SCSI_COMMAND:
					if (connection.getTargetSession().isNormalSession()) {
						...
						LOGGER.debug("scsiOpCode = " + scsiOpCode);//log SCSI Operation Code
						...
						if (scsiOpCode != null) {
							switch (scsiOpCode) {
								...
								case MODE_SELECT_6:
									stage = null;
									/*
									 * Since the MODE SENSE (6) command is supported
									 * (in order to accommodate the MS iSCSI Initiator),
									 * the jSCSI Target is also required to support the
									 * MODE SELECT (6) command.
									 * But since none of the tested initiators use it,
									 * support for it will be treated as optional.
									 */
									break;
								case MODE_SENSE_6:
						...
						if (scsiOpCode == null) {
							LOGGER.error("Unsupported SCSI OpCode 0x"
									+ Integer.toHexString(parser.getCDB().get(0) & 255)
									+ " in SCSI Command PDU.");
