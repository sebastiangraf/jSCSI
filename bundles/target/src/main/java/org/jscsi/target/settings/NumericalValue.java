 ...
 * {@link NumericalValueRange}. Objects of these two classes represent
 * single integers and integer intervals, respectively, and can be parsed
 * from any properly formatted <i>key=value</i> pair <i>value</i>, be they
 * formatted in decimal, hexadecimal, or Base64 notation.
 * <p>
 * Instances of {@link NumericalValue} can be intersected, i.e. that by
 * using either the {@link #intersect(NumericalValue)} or the
 * {@link #intersect(NumericalValue, NumericalValue)}, a
 * {@link NumericalValue} can be created which encompasses all integers
 * that are part of the range of values represented by the intersected
 * {@link NumericalValue} objects.
 * <p>
 * For example, if the interval [1,10] is intersected with the interval
 * [5,15], the result would be a {@link NumericalValueRange} representing
 * the interval [5,10]. Intersecting an interval with a single number would
 * return the number, but only if the number is part of the interval.
 * Generally, both methods return <code>null</code> if there is no overlap.
...
public abstract class NumericalValue {
	 ...
	 */
	public static final Pattern NUMERICAL_RANGE_PATTERN =
		Pattern.compile("(" + SINGLE_CONSTANT_REGEX + ")~(" + SINGLE_CONSTANT_REGEX + ")");
	 ...
	 * does not match any of the supported patterns (specified by the iSCSI
	 * standard).
	 */
	 ...
	 * @param value the {@link NumericalValue} to be intersected with this
	 * object
	 * @return a {@link NumericalValue} representing the intersection of
	 * this {@link NumericalValue} with the parameter
	 */
	...
	/**
	 * Returns a {@link NumericalValue} representing the intersection of
	 * the two parameters
	...
	/**
	 * Returns true if the passed {@link Integer} or {@link NumericalValue}
	 * lies completely inside the interval represented by this
	 * {@link NumericalValue}. If the parameter is not an {@link Integer} or
	 * a {@link NumericalValue}, the method will return <code>false</code>. 
	...
	/**
	 * Returns true if the passed integer lies completely
	 * inside the interval represented by this {@link NumericalValue}. 
