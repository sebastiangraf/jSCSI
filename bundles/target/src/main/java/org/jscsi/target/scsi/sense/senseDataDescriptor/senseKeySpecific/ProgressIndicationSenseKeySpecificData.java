 ...
 * progress of a previously issued SCSI command
...
public final class ProgressIndicationSenseKeySpecificData extends SenseKeySpecificData {
	 ...
	 * returned value is a numerator that has 65 536 (10000h) as its denominator.
	 * The progress indication shall be based upon the total operation.
	 * <p>
	 * The progress indication should be time related, however this is not an absolute
	 * requirement. (E.g., since format time varies with the number of defects encountered,
	 * etc., it is reasonable for the device server to assign values to various steps
	 * within the process. The granularity of these steps should be small enough to provide
	...
	/**
	 * The consctructor.
	 * @param senseKeySpecificDataValid <code>true</code> if and only if the
	 * <i>progressIndication</i> parameter is to be considered valid
	 * @param progressIndication specifies the progress, the integer must lie in the interval
	 * [0, 65536]
	 */
	...
	protected void serializeSpecificFields(ByteBuffer byteBuffer, int index) {
		...
		ReadWrite.writeTwoByteInt(byteBuffer, progressIndication,
				index + PROGRESS_INDICATION_FIELD_INDEX);
	}
}
