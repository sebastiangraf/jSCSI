...
public class InformationExceptionsControlModePage extends Page_0FormatModePage{
	...
	/**
	 * A disable exception control (DEXCPT) bit set to zero indicates the failure
	 * prediction threshold exceeded reporting shall be enabled. The method for
	 * reporting the failure prediction threshold exceeded when the DEXCPT bit is
	 * set to zero is determined from the MRIE field. A DEXCPT bit set to one
	 * indicates the device server shall disable reporting of the failure prediction
	 * threshold exceeded. The MRIE field is ignored when DEXCPT is set to one and
	 * EWASC is set to zero.
	 */
	private final boolean disableExceptionControl;
	
	/**
	 * A TEST bit set to one shall create a test device failure at the next interval
	 * time, as specified by the INTERVAL TIMER field, if the DEXCPT bit is set to
	 * zero. When the TEST bit is set to one, the MRIE and REPORT COUNT fields shall
	 * apply as if the TEST bit were zero. The test device failure shall be reported
	 * with the additional sense code set to FAILURE PREDICTION THRESHOLD EXCEEDED
	 * (FALSE). If both the TEST bit and the DEXCPT bit are one, the MODE SELECT
	 ...
	 * PARAMETER LIST. A TEST bit set to zero shall instruct the device server not to
	 * generate any test device failure notifications.
	 */
	private final boolean test;
	
	/**
	 * If the log errors (LOGERR) bit is set to zero, the logging of informational
	 * exception conditions by a device server is vendor specific. If the LOGERR bit
	 * is set to one, the device server shall log informational exception conditions.
	 */
	private final boolean logErrors;
	
	/**
	 * The value in the method of reporting informational exceptions (MRIE) field
	 * defines the method that shall be used by the device server to report
	 * informational exception conditions (see SPC3R23 table 257, p.294f). The priority of
	 ...
	 * in the REPORT COUNT field indicates there is no limit on the number of times
