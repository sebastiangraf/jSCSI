 ...
 * <p>
 * The <code>FORMAT UNIT</code> command requests that the device server
 * format the medium into application client accessible logical blocks as
 * specified in the number of logical blocks and logical block length
 * values received in the last mode parameter block descriptor in a
 * <code>MODE SELECT</code> command (see SPC-4). In addition, the device
 * server may certify the medium and create control structures for the
 ...
 * receiving a MODE SELECT command with a mode parameter block descriptor,
 * then the device server shall use the number of logical blocks and logical
 * block length at which the logical unit is currently formatted (i.e., no
 * change is made to the number of logical blocks and the logical block
 * length of the logical unit during the format operation).
 * <p>
 * <code>FORMAT UNIT</code> commands received by the jSCI Target will not
 * lead to any persistent changes of the virtual logical unit.
	...
	public void execute(ProtocolDataUnit pdu) throws IOException,
		...
		if (illegalFieldPointers != null) {
			...
			FixedFormatSenseData senseData = new FixedFormatSenseData(
					...
					illegalFieldPointers[0],//sense key specific data, only report first problem
					new AdditionalSenseBytes());//additional sense bytes
			
			responsePdu = TargetPduFactory.createSCSIResponsePdu(
					...
					0,//residualCount
					new ScsiResponseDataSegment(senseData, parser.getExpectedDataTransferLength()));//data segment
			...
			//carry out command
			/*
			 * If we were nice, we would have to get (we would actually have
			 * to save it first) the number of blocks and the block length
			 * requested by the initiator in the last MODE SENSE command and
			 * then change the logical block layout accordingly.
			 * 
			 * However, since the target is not required by the SCSI standard
			 * to make those changes ("The degree that the medium is altered
			 * by this command is vendor specific."), doing nothing is okay.
			 */
			
			responsePdu = createScsiResponsePdu(
