...
/**
 * MODE PAGEs are sent in response to successful <code>MODE SENSE</code>
 * SCSI commands. There are two different, command-specific formats for
 * MODE PAGEs - the PAGE_0 FORMAT and the SUB-PAGE FORMAT, represented by
 * the non-abstract children of this class, {@link Page_0FormatModePage}
 * and {@link SubPageFormatModePage}, respectively.
 * @author Andreas Ergenzinger
 */
public abstract class ModePage implements ISerializable {
	
	/**
	 * This value plus {@link #pageLength} equals the length in bytes of a
	...
	/**
	 * This value plus {@link #pageLength} equals the length in bytes of a
	 * MODE PAGE using the PAGE_0 FORMAT.
	 */
	private static final int PAGE_0_FORMAT_PAGE_LENGTH_MODIFIER = 2;
	
	/**
	 * When using the MODE SENSE command, a parameters saveable (PS) bit set
	 * to one indicates that the mode page may be saved by the logical unit
	 * in a nonvolatile, vendor specific location. A PS bit set to zero
	 * indicates that the device server is not able to save the supporte
	 *  parameters. When using the MODE SELECT command, the PS bit is reserved.
	 */
	private final boolean parametersSaveable;
	
	/**
	 * A SubPage Format (SPF) bit set to <code>false</code>/zero indicates
	 * that the page_0 mode page format is being used. A SPF bit set to
	 * <code>true</code>/one indicates that
	 * the sub_page mode page format is being used.
	 */
	...
	/**
	 * This value plus {@link #pageLength} equals the length in bytes of
	 * the MODE PAGE.
	 */
	 ...
	 * @param subPageFormat <code>true</code> if and only if the SUB-PAGE
	 * FORMAT is to be used
	 * @param pageCode specifies the kind of information is contained in
	 * the MODE PAGE
	 * @param pageLength the value of the PAGE LENGTH field
	 */
	public ModePage(final boolean parametersSaveable, final boolean subPageFormat,
			final int pageCode, final int pageLength) {
	...
	public final void serialize(final ByteBuffer byteBuffer, final int index) {
		...
		//(for convenience reasons from least significant to most significant field)
	 ...
	 * @param index the position of the first byte of the serialized object in
	 * the {@link ByteBuffer}
	 */
