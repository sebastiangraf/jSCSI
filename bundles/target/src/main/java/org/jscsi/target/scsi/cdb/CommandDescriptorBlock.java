 ...
 * Blocks of all sizes.
 * <p>
 * A Command Descriptor Blocks is a blocks of multiple bytes that contains
 * a {@link ScsiOperationCode}, specifying a command the SCSI initiator wants
 * the SCSI target to perform and related fields, which determine details
 * of the ordered task.
 * <p>
...
public abstract class CommandDescriptorBlock {
	...
	/**
	 * Not a CDB field. This array stores all {@link FieldPointerSenseKeySpecificData}
	 * that has to be sent back to the SCSI initiator as a reaction to the CDB
	 * {@link ByteBuffer} passed during initialization of this
	...
	/**
	 * Specifies if Normal ACA shall be used. A value of <code>true</code>
	 * is not supported by the jSCSI Target.
	 */
	 ...
	 * <p>
	 * The passed {@link ByteBuffer} parameter <b>must</b> have a capacity &ge; the length
	 * of the specific command descriptor block. Since this is assured by the
	 * {@link SCSICommandParser#getCDB()} method, which always returns {@link ByteBuffer}
	 * objects with capacity 16, no checks will be performed.
	...
	public CommandDescriptorBlock(ByteBuffer buffer) {
		...
		/*
		 * The above if block should never be entered, since unsupported operation codes
		 * in the CDB would have been discovered during TargetFullFeatureStage selection
		 * in TargetFullFeaturePhase.execute().
		 */
		
		//Control (NACA bit, all other fields are obsolete, reserved, or vendor specific)
	...
	/**
	 * The NACA (Normal ACA) bit specifies whether an auto contingent allegiance (ACA)
	 * is established if the command terminates with CHECK CONDITION status. A NACA bit
	 * set to one specifies that an ACA shall be established. A NACA bit set to zero
	 * specifies that an ACA shall not be established.
	 * @return <code>true</code> if the Normal ACA bit in the CDB's Control byte is set
	...
	/**
	 * Adds an instance {@link FieldPointerSenseKeySpecificData}, which
	 * specifies an illegal field by the position of its first byte and
	 * its first bit, to {@link #illegalFieldPointers}.
	 * @param byteNumber index of the first byte of the illegal field
	 ...
	 * {@link #illegalFieldPointers}. Initializes and grows the array
	 * if necessary.
