...
/**
 * A stage for processing <code>READ (6)</code> and <code>READ (10)</code>
 * SCSI commands.
...
public class ReadStage extends ReadOrWriteStage {
	...
	public void execute(ProtocolDataUnit pdu) throws IOException,
			InterruptedException, InternetSCSIException,
			SettingsException {
		...
		if (bytesSent < totalTransferLength - maxRecvDataSegmentLength) {
			 ...
			 */
			dataSegmentArray = connection.getDataInArray(
		...
		while (bytesSent < totalTransferLength - maxRecvDataSegmentLength) {
			...
			//create and send PDU
			responsePdu = TargetPduFactory.createDataInPdu(
					false,//finalFlag, not the last PDU with data payload in the sequence
					false,//acknowledgeFlag, ErrorRecoveryLevel == 0, so we never do that
		...
		responsePdu = TargetPduFactory.createDataInPdu(
				true,//finalFlag, last PDU in the sequence with data payload 
				false,//acknowledgeFlag, ErrorRecoveryLevel == 0, so we never do that
				false,//residualOverflowFlag
				false,//residualUnderflowFlag
		...
		if (!immediateData) {
			...
			responsePdu = TargetPduFactory.createSCSIResponsePdu(
					...
					0,//residualCount
					ScsiResponseDataSegment.EMPTY_DATA_SEGMENT);//empty ScsiResponseDataSegment
