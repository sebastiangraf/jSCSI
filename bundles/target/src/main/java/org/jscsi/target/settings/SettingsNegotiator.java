...
/**
 * This is an abstract parent-class to {@link ConnectionSettingsNegotiator}
 * and {@link SessionSettingsNegotiator}. The former one is in charge of all
 * {@link Entry} objects with connection-specific parameters, the latter is
 * in charge of all session-wide parameter {@link Entry} objects.
...
public abstract class SettingsNegotiator {
	 ...
	 * {@link #entries} or <code>null</code>.
	 * @see #backUpEntries
	...
	/**
	 * Returns the first {@link Entry} in the specified {@link Collection}
	 * for which {@link Entry#matchKey(String)} returns <code>true</code>.
	...
	/**
	 * Returns a deep copy of the passed {@link List} with exact copies
	 * of all contained {@link Entry} objects.
	 * 
	 ...
	 * either to roll back any changes that must not take effect, or in order
	 * to ensure that consecutive negotiations are carried out correctly.
	 * 
	 * @param commitChanges <code>true</code> if and only if the negotiated
	 * changes to the elements in {@link #entries} are to be remembered.
	 */
	protected final void commitOrRollBackChanges(final boolean commitChanges) {
		if (commitChanges)
			for (Entry e : entries)//allow renegotiation where allowed
	...
	/**
	 * Adds a properly initialized {@link Entry} object to {@link #entries}
	 * for every parameter managed by this {@link SettingsNegotiator}.
	 */
