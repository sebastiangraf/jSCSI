...
/**
 * The Caching mode page defines the parameters that affect the use of
 * the data cache by the storage medium.
 * @author Andreas Ergenzinger
 */
public final class CachingModePage extends Page_0FormatModePage {
	
	/**
	 * An initiator control (IC) enable bit set to one specifies that
	 * the device server use one of the following fields to control the
	 * caching algorithm rather than the device server’s own adaptive
	 * algorithm:
	 * <ol>
	 *  <li>
	 *   the NUMBER OF CACHE SEGMENTS field, if the SIZE bit is set
	 *   to zero; or
	 *  </li>
	 *  <li>
	...
	/**
	 * An abort pre-fetch (ABPF) bit set to one when the DRA bit is set
	 * to zero specifies that the device server abort a pre-fetch upon
	 * receipt of a new command. An ABPF bit set to one takes precedence
	 * over the value specified in the MINIMUM PRE-FETCH field. An ABPF
	 * bit set to zero when the DRA bit set to zero specifies that the
	 * termination of any active pre-fetch is dependent upon Caching mode
	 * page bytes 4 through 11 and is vendor specific.
	 */
	private final boolean abortPrefetch;
	
	/**
	 * A caching analysis permitted (CAP) bit set to one specifies that
	 * the device server perform caching analysis during subsequent
	 * operations. A CAP bit set to zero specifies that caching analysis
	 * be disabled (e.g., to reduce overhead time or to prevent
	 * nonpertinent operations from impacting tuning values).
	 */
	private final boolean cachingAnalysisPermitted;
	
	/**
	 * A discontinuity (DISC) bit set to one specifies that the device
	 * server continue the pre-fetch across time discontinuities (e.g.,
	 * across cylinders) up to the limits of the buffer, or segment, space
	 * available for the pre-fetch. A DISC bit set to zero specifies that
	 * pre-fetches be truncated or wrapped at time discontinuities.
	 */
	private final boolean discontinuity;
	
	/**
	 * A size enable (SIZE) bit set to one specifies that the CACHE SEGMENT
	 * SIZE field be used to control caching segmentation. A SIZE bit set
	 * to zero specifies that the NUMBER OF CACHE SEGMENTS field be used to
	 * control caching segmentation. Simultaneous use of both the number of
	 * segments and the segment size is vendor specific.
	 */
	private final boolean sizeEnable;
	
	/**
	 * A writeback cache enable (WCE) bit set to zero specifies that the
	 * device server shall complete a WRITE command with GOOD status only
	 * after writing all of the data to the medium without error. A WCE bit
	 * set to one specifies that the device server may complete a WRITE
	 * command with GOOD status after receiving the data without error and
	 ...
	 * device server shall interpret the MINIMUM PRE-FETCH field and the
	 * MAXIMUM PRE-FETCH field to be specified in terms of a scalar number that,
	 * when multiplied by the number of logical blocks to be transferred for the
	...
	/**
	 * A read cache disable (RCD) bit set to zero specifies that the device server
	 * may return data requested by a READ command by accessing either the cache
	 * or medium. A RCD bit set to one specifies that the device server shall
	 * transfer all of the data requested by a READ command from the medium (i.e.,
	 * data shall not be transferred from the cache).
	 */
	private final boolean readCacheDisable;
	
	/**
	 * The DEMAND READ RETENTION PRIORITY field specifies the retention priority
	 * the device server should assign for data read into the cache that has also
	 * been transferred to the data-in buffer.
	 * <table border="1">
	 *  <tr>
	 *   <th>Code</th>
	 *   <th>Description</th>
	 ...
	 * retention priority the device server should assign for data written
	 * into the cache that has also been transferred from the cache to the
	 * medium.
	 * <table border="1">
	 *  <tr>
	 *   <th>Code</th>
	 ...
	 * disabling of anticipatory pre-fetch on long transfer lengths. The
	 * value in this field is compared to the transfer length requested by
	 * a READ command. If the transfer length is greater than the disable
	 * pre-fetch transfer length, then an anticipatory pre-fetch is not done
	 * for the command. Otherwise the device server should attempt an
	 * anticipatory pre-fetch. If the DISABLE PRE-FETCH TRANSFER LENGTH field
	 * is set to zero, then all anticipatory pre-fetching is disabled for
	 ...
	 * field, the MINIMUM PRE-FETCH field, the MAXIMUM PRE-FETCH field, and
	 * the MAXIMUM PRE-FETCH CEILING field give an indication to the device
	 * server how it should manage the cache based on the most recent READ
	 * command. An anticipatory pre-fetch may occur based on other information.
	 * These fields are only recommendations to the device server and should
	 * not cause a CHECK CONDITION to occur if the device server is not able
	 ...
	 * pre-fetch regardless of the delays it might cause in processing subsequent
	 * commands. The field contains either:
	 * <ol>
	 ...
	 * <p>
	 * Pre-fetching shall always halt when it reaches the last logical block on the
	 * medium. Errors that occur during the pre-fetching operation shall not be
	 * reported to the application client unless the device server is unable to
	 * process subsequent commands correctly as a result of the error. In this case
	 * the error may be reported either as an error for that subsequent command, or
	 * as a deferred error, at the discretion of the device server and according to
	 * the rules for reporting deferred errors (see SPC-4).
	 * <p>
	 ...
	 * commands writing to more than one logical block, the device server
	 * shall write the logical blocks to the medium in ascending sequential
	 * order. An FSW bit set to zero specifies that the device server may
	 * reorder the sequence of writing logical blocks (e.g., in order to
	 * achieve faster command completion).
	 */
	private final boolean forceSequentialWrite;
	
	/**
	 * A logical block cache segment size (LBCSS) bit set to one specifies
	 * that the CACHE SEGMENT SIZE field units shall be interpreted as logical
	 * blocks. An LBCSS bit set to zero specifies that the CACHE SEGMENT SIZE
	 * field units shall be interpreted as bytes. The LBCSS shall not impact
	 * the units of other fields.
	 */
	 ...
	 * beyond the addressed logical block(s). A DRA bit set to zero specifies
	 * that the device server may continue to read logical blocks into the
	 * pre-fetch buffer beyond the addressed logical block(s).
	 */
	private final boolean disableReadAhead;
	
	/**
	 * An NV_DIS bit set to one specifies that the device server shall disable
	 * a non-volatile cache and indicates that a non-volatile cache is
	 * supported but disabled. An NV_DIS bit set to zero specifies that the
	 * device server may use a non-volatile cache and indicates that a
	 * non-volatile cache may be present and enabled.
	 */
	 ...
	 * The constructor.
	 * <p>
	 * The meaning of all parameters is described in the member descriptions
	...
	public CachingModePage(final boolean parametersSaveable, final boolean initiatorControl,
			final boolean abortPrefetch, final boolean cachingAnalysisPermitted,
			final boolean discontinuity, final boolean sizeEnable,
			final boolean writebackCacheEnable, final boolean multiplicationFactor,
			final boolean readCacheDisable, final int demandReadRetentionPriority,
			final int writeRetentionPriority, final int disablePrefetchTransferLength,
			final int minimumPrefetch, final int maximumPrefetch,
			final int maximumPrefetchCeiling, final boolean forceSequentialWrite,
			final boolean logicalBlockCacheSegmentSize, final boolean disableReadAhead,
			final boolean nonVolatileCacheDisabled, final int numberOfCacheSegments,
			final int cacheSegmentSize) {
