 ...
 * <p>
 * Only support for the default self-test feature, as required by SPC-3,
 * is implemented. Request for other types of self-test operations will
 * be declined.
...
public class SendDiagnosticStage extends TargetFullFeatureStage {
	...
	public void execute(ProtocolDataUnit pdu) throws IOException,
		...
		if (illegalFieldPointers != null) {
			...
			responsePdu = createFixedFormatErrorPdu(
					illegalFieldPointers,//senseKeySpecificData
			...
			/* 
			 * The self-test bit is 1, since request of a non-default self-test is not supported
			 * and would have led to illegalFieldPointer != null
			 * 
			 * A self-test (SELFTEST) bit set to one specifies that the device server shall
			 * perform the logical unit default self-test. If the self-test successfully
			 * passes, the command shall be terminated with GOOD status. If the self-test
			 * fails, the command shall be terminated with CHECK CONDITION status, with the
			 * sense key set to HARDWARE ERROR.
			 * 
