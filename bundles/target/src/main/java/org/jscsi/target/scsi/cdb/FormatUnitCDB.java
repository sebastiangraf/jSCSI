...
public final class FormatUnitCDB extends CommandDescriptorBlock {
	...
	/**
	 * The format protection information (FMTPINFO) field in combination
	 * with the PROTECTION FIELD USAGE field
	 */
	private byte formatProtectionInformation;
	
	/**
	 * A LONGLIST bit set to zero specifies that the parameter list,
	 * if any, contains a short parameter list header as defined in
	 * SBC-3. A LONGLIST bit set to one specifies that the parameter
	 * list, if any, contains a long parameter list header.
	 * <p>
	 * If the FMTDATA bit is set to zero, then the LONGLIST bit shall
	 * be ignored.
	 */
	private boolean longList;
	
	/**
	 * A format data (FMTDATA) bit set to zero specifies that no
	 * parameter list be transferred from the data-out buffer
	 * (i.e. the PDU's data segment is empty).
	 */
	private boolean formatData;
	
	/**
	 * A complete list (CMPLST) bit set to zero specifies that the
	 * defect list included in the FORMAT UNIT parameter list shall
	 * be used in an addition to the existing list of defects.
	 */
	private boolean completeList;
	
	/**
	 * If the FMTDATA bit is set to one, then the DEFECT LIST FORMAT
	 * field specifies the format of the address descriptors in the
	 * defect list.
	 */
	...
	public FormatUnitCDB(ByteBuffer buffer) {
		...
		if (!formatData) { 
			...
			/*
			 * If the FMTDATA bit is set to zero and the FMTPINFO field is not set to zero,
			 * then the device server shall terminate the command with CHECK CONDITION status
