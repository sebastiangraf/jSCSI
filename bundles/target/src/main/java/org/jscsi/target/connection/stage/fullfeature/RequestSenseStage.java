 ...
 * <p>
 * The <code>REQUEST SENSE</code> command requests that the device server
 * transfer {@link SenseData} to the application client.
	...
	public void execute(ProtocolDataUnit pdu) throws IOException,
		...
		if (illegalFieldPointers != null) {
			...
			if (cdb.getDescriptorFormat()) {
				...
				senseData = new DescriptorFormatSenseData(
				...
				senseData = new FixedFormatSenseData(
						false,//valid
			 ...
			 * carry out command
			 * 
			 * Sense data shall be available and cleared under the conditions defined in SAM-3.
			 * If the device server has no other sense data available to return, it shall return
			 * the sense key set to NO SENSE and the additional sense code set to NO ADDITIONAL
			 * SENSE INFORMATION. This will always be the case with the jSCSI Target.
			 */
			...
			if (cdb.getDescriptorFormat()) {
				...
				senseData = new FixedFormatSenseData(
						...
						additionalSense,//additional sense code and qualifier
						(byte)0,//field replaceable unit code
						null,//sense key specific data, only report first problem
