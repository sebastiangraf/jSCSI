 ...
 * are either declared by the iSCSI initiator or negotiated between the initiator
 * and the target a separate {@link Entry} takes care of processing the
 * respective <i>key=value</i> pair and returning the negotiated value, if
 * appropriate.
 * <p>
 * For brevity, the term "negotiated" will be used in the following in a way that
 * can either mean "declared or negotiated", unless the distinction is evident by
 * context.
...
public abstract class Entry {
	...
	/**
	 * This variable is used to detect illegal attempts to renegotiate a previously
	 * negotiated or declared text parameter.
	 * <p>
	 * This variable will be set back to <code>false</code> after each negotiation
	 * task (login phase, or text parameter negotiation stage). Renegotiation
	 * accross stages/tasks can be prevented by initializing the {@link #use}
	 ...
	 * @param use determines under which circumstances the parameter may be
	 * negotiated
	 * @param negotiationStatus indicates whether there is a default value or
	 * if the parameter must be negotiated
	...
	/**
	 * Logs an error message containing all {@link #keySet} keys as well as
	 * the passed {@link String} parameter and indicates an unsuccessful
	 * negotiation by setting {@link #negotiationStatus} to
	...
	/**
	 * Parses the passed {@link String} parameter and returns a sub-class-specific
	 * {@link Object} which represents the the specified <i>value</i> part a
	 * <i>key=value</i> pair.
	 * @param values the <i>value</i> part of a <i>key=value</i> pair
	 * @return sub-class-specific {@link Object} or <code>null</code> if
	 * the parameter violated the expected format
	 */
	 ...
	 * parameter.
	 * @param loginStage specifying the current stage or phase of the connection
	 * whose parameters are to be negotiated 
	 * @param leadingConnection <code>true</code> if the connection is the first
	 * connection in its session, <code>false</code> if not
	 * @param initialPdu <code>true</code> if the <i>key=value</i> pair parameters
	 * have been sent in the first login {@link ProtocolDataUnit} from the
	 ...
	 * @param responseKeyValuePairs where the reply <i>key=value</i> pair will be
	 ...
	 * <p>
	 * This method must be used at the end of each negotiation task, i.e. at
	 * the end of the login phase and the FFP text negotiation stage.
	 */
	 ...
	 * @throws OperationNotSupportedException if {@link #value} is not
	 * of the boolean type
	 */
	 ...
	 * @param values a sub-class-specific {@link Object},
	 * representing a single, a range, or a list of values sent by the
	 * initiator
	 ...
	 * {@link #value}.
	 * @param values sub-class-specific representation of a single <i>value</i>
	 ...
	 * tries to select a value from that offer. If none of the offered values
	 * is supported by the jSCSI Target, <code>null</code> is returned, otherwise
	 * the selection is accepted as the new {@link #value} and returned as a
	 * {@link String}.
	 * {@link #value}.
	 * @param values a sub-class-specific {@link Object}, representing a list, a
	 * range, or a single legal parameter value offered by the initiator
	 * @return the final, negotiated value or <code>null</code>, if the initiator's
	 * offer does not overlap with the values supported by the jSCSI Target
	 */
