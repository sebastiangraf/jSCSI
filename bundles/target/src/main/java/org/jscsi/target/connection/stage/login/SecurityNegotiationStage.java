...
public final class SecurityNegotiationStage extends TargetLoginStage {
	...
	public void execute(ProtocolDataUnit initialPdu) throws IOException, InterruptedException, InternetSCSIException, DigestException, SettingsException {
		...
		do {//while initiator is not willing and not authorized to transit to next stage
			...
			//extract available AuthMethod key-value pair, so that settings can finish
			...
			if (!authenticated) {//authentication part one
				for (int i = 0; i < requestKeyValuePairs.size(); ++i) {
					...
					if (TextKeyword.AUTH_METHOD.equals(split[0])) {
						...
						requestKeyValuePairs.remove(i--);//correct for shifted indices
						//no break here to catch all authMethodKeyValuePairs in else block
					} else if (isAuthenticationKey(split[0])) {
						//move key-value pair to authMethodKeyValuePairs
						authMethodKeyValuePairs.add(requestKeyValuePairs.remove(i--));//correct for shifted indices
					}
				}
				if (authMethodValues == null) {//missing AuthMethod key
					sendRejectPdu(LoginStatus.MISSING_PARAMETER);//require AuthMethod to be specified in first PDU sequence
					//close connection
			...
			final Vector<String> responseKeyValuePairs = new Vector<String>();//these will be sent back
			...
			if (!authenticated) {
				if (authMethodValues.contains(TextKeyword.NONE)) {
					...
					//TODO support CHAP (and use String authMethodKeyValuePairs)
	...
	/**
	 * Checks if a the parameter is the key of an AuthMethod Key, which means one of the
	 * following (where &#60;key&#62; depends on the AuthMethod prefix):
	 * <ul>
	 * <li>CHAP_&#60;key&#62;/il>
	 * <li>KRB_&#60;key&#62;/il>
	 * <li>SPKM_&#60;key&#62;/il>
	 ...
	 * @return <code>true</code> if the String is an AuthMethod key, <code>false</code> if
	 * it is not.
	 */
	private final boolean isAuthenticationKey(final String key) {
		...
		if ("CHAP_".matches(fiveChars) || "KRB_".matches(fourChars) ||
				"SPKM_".matches(fiveChars) || "SRP_".matches(fourChars)
