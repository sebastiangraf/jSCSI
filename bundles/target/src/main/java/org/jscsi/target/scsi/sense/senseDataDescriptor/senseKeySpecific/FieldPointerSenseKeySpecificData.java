...
public final class FieldPointerSenseKeySpecificData extends SenseKeySpecificData {
	...
	/**
	 * A command data (C/D) bit set to one indicates that the illegal parameter is
	 * in the CDB. A C/D bit set to zero indicates that the illegal parameter is
	 * in the data parameters sent by the application client in the Data-Out Buffer.
	 */
	 ...
	 * specifies which bit of the byte designated by the FIELD POINTER field is in
	...
	/**
	 * The FIELD POINTER field indicates which byte of the CDB or of the parameter
	 * data was in error. Bytes are numbered starting from zero, as shown in the
	 * tables describing the commands and parameters. When a multiple-byte field is
	 * in error, the field pointer shall point to the first byte (i.e., the left-most
	 * byte) of the field. If several consecutive bytes are reserved, each shall be
	 * treated as a single-byte field.
	 */
	private final short fieldPointer;
	
	public FieldPointerSenseKeySpecificData(final boolean senseKeySpecificDataValid,
			final boolean commandData, final boolean bitPointerValid,
