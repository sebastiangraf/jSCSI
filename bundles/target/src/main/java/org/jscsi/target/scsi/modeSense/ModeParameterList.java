 ...
 * <p>
 * This class uses the builder pattern to minimize the number of
 * constructors and to avoid constructor use with a lot of <code>null</code>
 * parameters.
...
public final class ModeParameterList implements IResponseData {
	...
	/**
	 * Returns a new {@link ModeParameterList} instance based the
	 * variables of the <i>builder</i> object. If these variables are
	 * mutually exclusive, then this method will return <code>null</code>
	 * instead.
	 * @param builder contains all necessary information to create a
	...
	public static ModeParameterList build(final ModeParameterListBuilder builder) {
		...
		if (builder.logicalBlockDescriptors == null)
			...
			blockDescriptorLength =
				builder.logicalBlockDescriptors.length * singleLbdLength;
		}
		// - calculate mode data length
		int modeDataLength;
		// -- calculate contribution of the header without bytes of MODE DATA LENGTH field
		...
		/*
		 * It might be nice to check the values for overflow here,
		 * however this is not necessary, since the length of all
		 * available/returned elements will always be less than 256.
		 */
		
		ModeParameterHeader modeParameterHeader;
		if (builder.headerType == HeaderType.MODE_PARAMETER_HEADER_6)
			modeParameterHeader = new ModeParameterHeader6(
					modeDataLength, blockDescriptorLength);
		else
			modeParameterHeader = new ModeParameterHeader10(
					modeDataLength, blockDescriptorLength, builder.longLba);
		
		//create and return the ModeParameterList
		return new ModeParameterList(modeParameterHeader,
				builder.logicalBlockDescriptors,
				builder.modePages);
	}
	...
	public int size() {
		...
		if (logicalBlockDescriptors != null && logicalBlockDescriptors.length > 0)
			size += logicalBlockDescriptors[0].size() * logicalBlockDescriptors.length;//all have the same size
		if (modePages != null)
			for (ModePage mp : modePages)
				size += mp.size();
		return size;
