...
public final class TextParameter {
	...
	/**
	 * Returns the <i>key-value</i> pairs contained in a null character-separated text data segment
	...
	public static Vector<String> tokenizeKeyValuePairs(final String keyValuePairs) {
		...
		for (int i = 0; i < split.length; ++i)
			if (split[i].length() > 0)//does not mean key-value pair is RFC-conform
				result.add(split[i]);
		return result;
	}
	 ...
	 * {@link Collection} to a null-character-separated {@link String} that
	 * can be sent as a text parameter data segment. 
	 ...
	 * <i>prefix</i>, but of course only if the beginning of <i>string</i>
	 * does equal <i>prefix<i>. If <i>prefix</i> is not a prefix of
	 * <i>string</i>, <code>null</code> is returned.
	...
	/**
	 * Splits a <i>key=value</i> pair and returns an array with the
	 * separated <i>key</i> and <i>value</i> parts.
	 * <p>
	 * If the parameter does not match this required pattern, then
	 * <code>null</code> will be returned. 
	 * @param keyValuePair a {@link String} with a <i>key</i> prefix of length
	 * > 0, a '=' in the middle and a <i>value</i> suffix of length > 0
	 * @return array with the separated <i>key</i> and <i>value</i> parts
	 * or <code>null</code>.
	 */
	...
	/**
	 * Splits a String of (one or more) values at the ',' signs and returns the values
	...
	/**
	 * Returns an array of Strings containing only those String values present in
	 * both input String arrays <i>a</i> and <i>b</i>.
	 * <p>
	...
	public static String[] intersect(String[] a, String[] b) {
		if (a == null || b == null)
			return null;
		final int maxLength = Math.max(a.length, b.length);//prevent growing of the ArrayList
	...
	/**
	 * A methods for parsing boolean values from the <i>value</i> part of a <i>key=value</i>
	 * pair String. If <i>value</i> equals <i>"Yes"</i>, then <code>true</code> will be returned,
	 * if <i>value</i> equals <i>"No"</i>, then <code>false</code> will be returned. In all other
	 * cases the method will return <code>null</code>.
	...
	/**
	 * Checks if the <i>value</i> parameter is a properly formatted String value, i.e.
	 * if it only contains the allowed characters. The list of legal characters is specified
	 * in RFC3720, section 5.1. All characters from that list except for '~' and the null
	 * character are considered legitimate.
	 * <p>
	 * If those constraints are violated, the method returns <code>null</code>.
	 * @param value the <i>value</i> part of a <i>key=value</i> pair, which is a String text
	 * parameter
	 ...
	 * <i>key=value</i> pair as required by iSCSI text parameter
	 * negotiation and returns the result.
