...
/**
 * Instances of this class are used by {@link Entry} objects to determine
 * during which circumstances a parameter may be negotiated or declared.
 * <p>
 ...
 *  <li>if the <i>key-value</i> pair relating to the parameter was part
 *  of the first {@link ProtocolDataUnit} sent over the connection</i>
 * </ul>
...
public enum Use {
	...
	/**
	 * The parameter may be negotiated by any connection in the
	 * {@link LoginOperationalParameterNegotiationStage} or in the
	 * Full Feature Phase (specifically the {@link TextNegotiationStage}).
	 */
	LOPNS_AND_FFP,
	/**
	 * The parameter may be negotiated by any connection in the
	...
	/**
	 * This method can be used for checking if a specified {@link Use}
	 * permits negotiating its associated parameter given a situation
	 * described by several parameters.
	 * @param use describes the conditions
	 * @param loginStage specifies the stage of phase
	 * @param leadingConnection <code>true</code> if and only if the connection is
	 * the leading connection of its session
	 * @param initialPdu <code>true</code> if and only if the
	 * {@link ProtocolDataUnit} is the first PDU sent over the connection
	 * @return <code>true</code> if and only if all requirements of the <i>use</i>
	 * parameter have been met
	 */
	private static boolean checkUse(final Use use,
			final LoginStage loginStage,
			final boolean leadingConnection, final boolean initialPdu) {
		switch (use) {
			case LEADING_LOPNS:
				if (leadingConnection && loginStage ==
			...
			case INITIAL_AND_FFP:
				...
				//fall through
			case INITIAL:
				if (initialPdu && (loginStage == LoginStage.SECURITY_NEGOTIATION || 
	...
	/**
	 * This method can be used for checking if this {@link Use} instance
	 * permits negotiating its associated parameter given a situation
	 * described by the passed parameters.
	 * @param loginStage specifies the stage of phase
	 * @param leadingConnection <code>true</code> if and only if the connection is
	 * the leading connection of its session
	 * @param initialPdu <code>true</code> if and only if the
