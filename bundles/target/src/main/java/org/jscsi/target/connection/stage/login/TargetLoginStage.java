 ...
 * {@link TargetLoginPhase} (see {@link TargetConnection} for a
 * description of the relationship between sessions, connections, phases
 * and sessions), namely the {@link LoginOperationalParameterNegotiationStage}
 * and the {@link SecurityNegotiationStage}.
 * <p>
 ...
 * the stage.
 * <p>
 * Of equal importance is the {@link #getNextStageNumber()} method, which
 * must be used to find out which stage or phase will follow this one.
...
public abstract class TargetLoginStage extends TargetStage {
	 ...
	 * Sent with the first Login Request PDU.
	 */
	 ...
	 * <p>
	 * This value is initialized with <code>null</code>, and will be changed
	 * in {@link #execute(ProtocolDataUnit)}, if the stage was finished
	 * successfully.
	 * @see #stageNumber
	 */
	 ...
	 * @param stageNumber the stage number used for describing this stage in
	 * Login Request PDUs
	 */
	public TargetLoginStage(
			
			final TargetLoginPhase targetLoginPhase,
			final LoginStage stageNumber) {
	 ...
	 * {@link LoginRequestParser#isContinueFlag()} and returns the
	 * concatenated content of the text data segments.
	...
	InternetSCSIException, IOException, SettingsException, InterruptedException {
	...
	throws InternetSCSIException, InterruptedException, IOException, DigestException, SettingsException {
		...
		while(sequenceLength <= Target.config.getInMaxRecvTextPduSequenceLength()) {
			...
			ReadWrite.appendTextDataSegmentToStringBuffer(
					pdu.getDataSegment(),
					stringBuilder);
			...
			if (parser.isContinueFlag()) {
				...
				pdu = TargetPduFactory.createLoginResponsePdu(
						false,//transitFlag
						false,//continueFlag
	 ...
	 * @param nextStage indicates if the target is willing to transition
	...
	/**
	 * Sends a Login Response PDU informing the initiator that an error
	 * has occurred and that the connection must be closed.
	...
	throws InterruptedException, IOException, InternetSCSIException {
		final ProtocolDataUnit rejectPDU = TargetPduFactory.createLoginResponsePdu(
				false,//transit flag
				false,//continueFlag
				stageNumber,//currentStage
				stageNumber,//nextStage
	...
	/**
	 * Returns a stage number describing which stage of phase must follow
	 * this stage, or <code>null</code> if the initiator is not allowed to
