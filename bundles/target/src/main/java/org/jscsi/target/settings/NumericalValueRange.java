...
/**
 * This is a {@link NumericalValue} sub-class for representing integer intervals.
 * Each {@link NumericalValueRange} is defined by two integers - {@link #min}
 * and {@link #max} - the lower and the upper bound of the represented interval.
...
public final class NumericalValueRange extends NumericalValue {
	...
	/**
	 * The build method for creating {@link NumericalValueRange} objects.
	 * The specified <i>min</i> and <i>max</i> parameters are going to be used
	 * for initializing the {@link #min} and {@link #max} variables of the new
	 ...
	 * boundaries must be separated by a '~' character and the the leading integer
	 * must not be larger than the trailing one. If these format requirements are
	...
	public static final NumericalValueRange parseNumericalValueRange(final String value) {
		...
		final SingleNumericalValue minVal =
			SingleNumericalValue.parseSingleNumericValue(numbers[0]);
		final SingleNumericalValue maxVal =
			SingleNumericalValue.parseSingleNumericValue(numbers[1]); 
	...
	/**
	 * Returns true if the passed {@link NumericalValue} lies completely
