...
public final class SessionSettingsNegotiator extends SettingsNegotiator {
	...
	/**
	 * A counter that provides a unique, ordered identifying value for
	...
	/**
	 * Blocks until the {@link #lock()} has been acquired and returns
	 * <code>true</code> or returns <code>false</code> if the method
	 * returned prematurely due to an interrupt.
	 * <p>
	 * The usual safeguards with using {@link Lock} objects should be
	 * applied, namely using <code>try ...  catch ... finally ...</code>
	 * blocks to make sure the lock is always released.
	 ...
	 * <i>key</i> identifying a session-wide parameter, or <code>null</code>
	 * if no such {@link Entry} can be found.
	 * @param key identifies an {@link Entry} responsible for a session-wide
	 * parameter
	...
	protected void initializeEntries() { 
		 ...
		 * order. Yes indicates that the data PDUs within a sequence have to
		 * be at continuously increasing addresses and that overlays are
		 * forbidden.
		 */
		entries.add(new BooleanEntry(
				new KeySet(TextKeyword.DATA_PDU_IN_ORDER),//keySet
		 ...
		 */
		entries.add(new BooleanEntry(
				new KeySet(TextKeyword.DATA_SEQUENCE_IN_ORDER),//keySet
				Use.LEADING_LOPNS,//use
				NegotiationStatus.DEFAULT,//negotiationStatus
				true,//negotiationValue
				BooleanResultFunction.OR,//resultFunction
		...
		entries.add(new NumericalEntry(
				new KeySet(TextKeyword.DEFAULT_TIME_2_RETAIN,
		 ...
		 * reinstatement after a connection termination or a connection reset.
		 * A value of zero means that task reassignment can be done immediately.
		 */
		entries.add(new NumericalEntry(
				new KeySet(TextKeyword.DEFAULT_TIME_2_WAIT,
						TextKeyword.TIME_2_WAIT),//keySet
				NegotiationType.NEGOTIATED,//negotiationType
				Use.LEADING_LOPNS,//use
				NegotiationStatus.DEFAULT,//negotiationStatus
				2,//negotiationValue
		 ...
		 */
		entries.add(new NumericalEntry(
				new KeySet(TextKeyword.ERROR_RECOVERY_LEVEL),//keySet
				NegotiationType.NEGOTIATED,//negotiationType
				Use.LEADING_LOPNS,//use
				NegotiationStatus.DEFAULT,//negotiationStatus
				0,//negotiationValue
		...
		/*
		 * Maximum SCSI payload, in bytes, of unsolicited data an initiator
		 * may send to the target. Includes immediate data and a sequence of
		 * unsolicited Data-Out PDUs. Zero (don't care) can be used.
		 * Must be <= MaxBurstLength.
		 */
		entries.add(new NumericalEntry(
				new KeySet(TextKeyword.FIRST_BURST_LENGTH),//keySet
				NegotiationType.NEGOTIATED,//negotiationType
				Use.LEADING_LOPNS,//use
				NegotiationStatus.DEFAULT,//negotiationStatus
		 ...
		 */
		entries.add(new BooleanEntry(
				new KeySet(TextKeyword.IMMEDIATE_DATA),//keySet
				Use.LEADING_LOPNS,//use
				NegotiationStatus.DEFAULT,//negotiationStatus
				true,//negotiationValue
		 ...
		 */
		entries.add(new BooleanEntry(
				new KeySet(TextKeyword.INITIAL_R_2_T),//keySet
				Use.LEADING_LOPNS,//use
				NegotiationStatus.DEFAULT,//negotiationStatus
				true,//negotiationValue
				BooleanResultFunction.OR,//resultFunction
		...
		entries.add(new StringEntry(
				new KeySet(TextKeyword.INITIATOR_ALIAS),//keySet
		 ...
		 */
		entries.add(new StringEntry(
				new KeySet(TextKeyword.INITIATOR_NAME),//keySet
				NegotiationType.DECLARED,//negotiationType
				Use.INITIAL,//use
				NegotiationStatus.NOT_NEGOTIATED,//negotiationStatus
				null,//supportedValues, anything goes
		 ...
		 */
		entries.add(new NumericalEntry(
				new KeySet(TextKeyword.MAX_BURST_LENGTH),//keySet
				NegotiationType.NEGOTIATED,//negotiationType
				Use.LEADING_LOPNS,//use
				NegotiationStatus.DEFAULT,//negotiationStatus
		...
		/*
		 * The initiator and target negotiate the maximum number of
		 * connections that can be requested or are acceptable.
		 */
		entries.add(new NumericalEntry(
				new KeySet(TextKeyword.MAX_CONNECTIONS),//keySet
				NegotiationType.NEGOTIATED,//negotiationType
				Use.LEADING_LOPNS,//use
				NegotiationStatus.DEFAULT,//negotiationStatus
		 ...
		 */
		entries.add(new NumericalEntry(
				new KeySet(TextKeyword.MAX_OUTSTANDING_R_2_T),//keySet
				NegotiationType.NEGOTIATED,//negotiationType
				Use.LEADING_LOPNS,//use
				NegotiationStatus.DEFAULT,//negotiationStatus
				1,//negotiationValue (default value)
		 ...
		 */
		entries.add(new StringEntry(
				new KeySet(TextKeyword.SESSION_TYPE),//keySet
				NegotiationType.DECLARED,//negotiationType
				Use.INITIAL,//use
				NegotiationStatus.DEFAULT,//negotiationStatus
	...
	/**
	 * Updates {@link #sessionSettingBuilderComponent} with the currently
	...
	protected void updateSettingsBuilderComponent() {
		...
		sessionSettingBuilderComponent =
			new SessionSettingsBuilderComponent(
	...
	public boolean checkConstraints() {
		try {
			...
			final int firstBurstLength =
				getEntry(TextKeyword.FIRST_BURST_LENGTH).getIntegerValue();
			final int maxBurstLength =
				getEntry(TextKeyword.MAX_BURST_LENGTH).getIntegerValue();
