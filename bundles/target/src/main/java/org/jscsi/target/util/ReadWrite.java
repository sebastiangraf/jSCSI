...
/**
 * This utility class contains static methods for reading/writing integers
 * of various lengths and character strings from/to {@link ByteBuffer} objects
 * and byte arrays.   
...
public final class ReadWrite {
	...
	/**
	 * Reads a specified byte from a {@link ByteBuffer} and returns its
	...
	/**
	 * Reads a specified byte from a byte array and returns its
	 * value as an unsigned integer.
	...
	/**
	 * Reads an unsigned 2-byte integer from two consecutive
	 * big-endian-ordered bytes of a {@link ByteBuffer} object and
	 * returns the value.
	...
	/**
	 * Reads a (signed) 4-byte integer from four consecutive
	 * big-endian-ordered bytes of a {@link ByteBuffer} object and
	 * returns the value.
	 * @param buffer the {@link ByteBuffer} containing the bytes
	 * @param start the index position of the most-significant byte
	 ...
	 * all but the last ByteBuffer will have <i>capacity() = bufferSize</i>,
	 * the last one will contain the remaining String characters.
	...
	public static ByteBuffer[] stringToTextDataSegments(String string, int bufferSize) {
		...
		for (int i = 0; i < numberOfParts; ++i) {
			if (bytesRemaining > bufferSize) {
				segments[i] = stringToByteBuffer(
						string.substring(i * bufferSize, (i + 1) * bufferSize));
			} else {
				segments[i] = stringToByteBuffer(
						string.substring(i * bufferSize));
			}
		}
		return segments;
	}
	...
	/**
	 * Appends the content of a {@link ProtocolDataUnit}'s (text) data
	 * segment to a {@link StringBuilder};
	...
	/**
	 * Writes the given <i>value</i> to the <i>buffer</i> in big-endian format, with
	 * the index position of the most significant byte being <i>start</i>.
	...
	/**
	 * Writes the given <i>value</i> to the <i>buffer</i> in big-endian format, with
	 * the index position of the most significant byte being <i>index</i>.
	 * @param value the integer to write to the ByteBuffer
	 * @param buffer where the integer will be stored
	 ...
	 * @param index the position of the most significant byte in the
	 * {@link ByteBuffer}
	 */
	public static void writeTwoByteInt(final ByteBuffer buffer, final int value,
			final int index) {
		buffer.position(index);
	 ...
	 * {@link ByteBuffer}
	 */
	public static void writeThreeByteInt(final ByteBuffer buffer, final int value,
			final int index) {
	...
	/**
	 * Reads an unsigned 4-byte integer from four consecutive
	 * big-endian-ordered bytes of a {@link ByteBuffer} object and
	 * returns the value.
	 * @param buffer the {@link ByteBuffer} containing the bytes
	 * @param start the index position of the most-significant byte
