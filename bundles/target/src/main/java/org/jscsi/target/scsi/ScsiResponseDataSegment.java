...
/**
 * Instances of this class represent data that may be sent in a SCSI
 * Command Response PDU.
 * <p>
 * It may contain either {@link SenseData}, {@link IResponseData}, or
 * (rarely) both. Some SCSI commands require that no response data is sent
 * (i.e. data segment length = 0) - in these cases the
 * {@link #EMPTY_DATA_SEGMENT} shall be used.
 * <p>
 * Since the target must honor the buffer size the initiator has allocated
 * for data returned in the data segment, the {@link #serialize()} method
 * will only write as many bytes to the passed {@link ByteBuffer} as
...
public final class ScsiResponseDataSegment {
	...
	/**
	 * The number of bytes the initiator has allocated for data sent in
	 * the data segment, i.e. the maximum number of bytes of this
	 * object that may be transmitted.
	 */
	 ...
	 * @param allocationLength number of bytes the initiator has allocated
	 * for data sent in the data segment
	 */
	...
	/**
	 * Creates a {@link ScsiResponseDataSegment} with both {@link SenseData}
	 * and {@link IResponseData}.
	...
	/**
	 * Returns a {@link ByteBuffer} containing a serialized representation
	 * of this object.
	 * @return a {@link ByteBuffer} containing a serialized representation
	 * of this object
	 */
	public ByteBuffer serialize() {
		...
		if (responseData != null)
			responseData.serialize(buffer,
					SENSE_LENGTH_FIELD_LENGTH + senseLength);
		
		
		
		if (allocationLength > 0 && buffer.capacity() > allocationLength) {
			/*
			 * If the allocation length variable is valid and the response
			 * data segment is larger than the data-in buffer allocated by
			 * the initiator, limit the number of bytes returned.
			 */
			 buffer.position(0);
	 ...
	 * @return length of the {@link ScsiResponseDataSegment}, without considering
	 * any limitations due to {@link #allocationLength}
	 */
	 ...
	 * calls. The method returns <code>true</code> if and only if the
	 * serialized length of this objects exceeds {@link #allocationLength}. 
	 * @return <code>true</code> if and only if the serialized length of 
	 * this objects exceeds {@link #allocationLength} 
	 */
	...
	/**
	 * Returns the number of bytes that had to be cropped due to
	 * the total length of all contained fields exceeding the
