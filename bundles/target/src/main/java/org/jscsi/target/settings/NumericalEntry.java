 ...
 * <p>
 * During text parameter negotiation, numerical parameters are negotiated
 * by sending a single integer <i>value</i> as part of a <i>key=value</i>
 * pair. Based on the values permitted for the specific <i>key</i>
 * ({@link #protocolValueRange}), the {@link #resultFunction} and the
 * {@link #negotiationValue}, the jSCSI Target will choose a single value
 * as the negotiation outcome.
 * <p>
 * For declared parameters this process is less complex, any proposed value
 * from the {@link #protocolValueRange} will be silently accepted.
 * <p>
...
public final class NumericalEntry extends Entry {
	...
	/**
	 * <code>true</code> means that a {@link #negotiationValue} of <code>0</code>
	 * allows the jSCSI Target to choose any value from the resulting range.
	 * <code>false</code> means that {@link #negotiationValue} must always be
	 * treated as an upper or lower boundary.
	 */
	 ...
	 * @param negotiationValue together with the <i>resultFunction</i> and the
	 * <i>protocolValueRange</i> parameters this value indicates the values
	 ...
	 * @param zeroMeansDontCare if <code>true</code> and <i>negotiationValue</i>
	 * equals <code>0</code> then <i>negotiationValue</i> does not serve as an
	 * upper or lower boundary to the values the jSCSI Target will accept
	 */
	public NumericalEntry(final KeySet keySet,
			...
			final boolean zeroMeansDontCare) {
	...
	protected String processNegotiation(final Object values) {
		...
		if (zeroMeansDontCare && val == 0)//pick value desired by target
			value = negotiationValue;
		else //pick value based on result function and offer
	...
	public Entry copy() {
		final NumericalEntry e = new NumericalEntry(keySet, negotiationType,
				...
				protocolValueRange, resultFunction, (Integer)value,
				zeroMeansDontCare);
