...
public abstract class TargetFullFeatureStage extends TargetStage {
    ...
    /**
     * Creates a PDU with {@link FixedFormatSenseData} that must be sent to the initiator after receiving a Command
     * Descriptor Block with an illegal field.
     * 
     * @param senseKeySpecificData contains a list of all illegal fields
     * @param additionalSenseCodeAndQualifier provides more specific information about the cause of the check condition
     * @param initiatorTaskTag used by the initiator to identify the task
     * @param expectedDataTransferLength the amount of payload data expected by the initiator (i.e. allocated buffer
     *            space)
     * @return the error PDU
     */
    protected static final ProtocolDataUnit createFixedFormatErrorPdu (final FieldPointerSenseKeySpecificData[] senseKeySpecificData, final AdditionalSenseCodeAndQualifier additionalSenseCodeAndQualifier, final int initiatorTaskTag, final int expectedDataTransferLength) {
        ...
        final ScsiResponseDataSegment dataSegment = new ScsiResponseDataSegment(senseData, expectedDataTransferLength);
     ...
     * @return the error PDU
     */
    protected static final ProtocolDataUnit createFixedFormatErrorPdu (final FieldPointerSenseKeySpecificData[] senseKeySpecificData, final int initiatorTaskTag, final int expectedDataTransferLength) {
        return createFixedFormatErrorPdu(senseKeySpecificData, AdditionalSenseCodeAndQualifier.INVALID_FIELD_IN_CDB, initiatorTaskTag, expectedDataTransferLength);
    }

    /**
     * Creates a SCSI Response PDU with a length zero data segment. Objects created with this method can be used as
     * replies in task terminating with {@link SCSIStatus#GOOD} which do not require additional data to be transfered,
     * or for creating follow-up PDU with {@link SCSIStatus#CHECK_CONDITION} status sent after Data-In PDUs with sense
     * data.
     * 
     * @param status the SCSI status of the task
     * @param initiatorTaskTag used by the initiator to identify the task
     * @param expectedDataTransferLength total amount of payload data in bytes expected by the initiator
     * @param responseDataSize actual amount of payload data in bytes sent by the target
     * @return the SCSI Response PDU
     */
    protected static final ProtocolDataUnit createScsiResponsePdu (final SCSIStatus status, final int initiatorTaskTag, final int expectedDataTransferLength, final int responseDataSize) {
    ...
    /**
     * Sends a two byte sequence of a Data-In and a SCSI Response PDU with the specified <i>responseData</i> payload to
     * the initiator.
     * 
     * @param initiatorTaskTag used by the initiator to identify the task
     * @param expectedDataTransferLength the total amount of payload data in bytes expected by the initiator
     *            <p>
     ...
     */
    protected final void sendResponse (final int initiatorTaskTag, final int expectedDataTransferLength, final IResponseData responseData) throws InterruptedException , IOException , InternetSCSIException {
