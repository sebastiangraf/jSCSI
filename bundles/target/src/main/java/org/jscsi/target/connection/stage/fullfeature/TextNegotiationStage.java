...
/**
 * A stage for processing requests by the initiator for a list of 
 * all targets available through the iSCSI portal and for negotiating
 * connection parameters in the full feature phase.
 * <p>
...
public final class TextNegotiationStage extends TargetFullFeatureStage {
	...
	private static final Logger LOGGER =
		Logger.getLogger(TextNegotiationStage.class);
	...
	public void execute(ProtocolDataUnit pdu) throws IOException,
		...
		if (requestKeyValuePairs != null) {
			...
			if (sendTargetsValue != null) {
				 ...
				 * (and it does no harm to support it in the operational session).
				 * 
				 * <nothing> must be supported in the operational session and
				 * will only return info on the target the initiator is connected
				 * to, this means no info is returned in discovery session.
				 * 
				 ...
				 * <nothing>      |    fail   |    TA     |
				 * 
				 * TN stands for TargetName, TA for TargetAddress
				 * "fail" means no text response.
				 */
				...
				final boolean sendTargetAddress = //see upper table
					(!normal && sendTargetsValue.equals(TextKeyword.ALL)) ||
					(sendTargetsValue.equals(Target.config.getTargetName())) ||
				 ...
				 * that TargetName.
				 * 
				 * Since there is just one target, there will only be one
				 * target record.
				 * 
			...
			replyDataSegment = ReadWrite.stringToTextDataSegments(
					replyString,
					settings.getMaxRecvDataSegmentLength())[0];//definitely fits into one data segment
		}
		
		//send reply
		final ProtocolDataUnit responsePdu = TargetPduFactory.createTextResponsePdu(
				true,//finalFlag
				false,//continueFlag
				0,//logicalUnitNumber
				initiatorTaskTag,
