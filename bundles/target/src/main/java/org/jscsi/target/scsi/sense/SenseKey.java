 ...
 * The field's length is 4 bits.
 * @author Andreas Ergenzinger
 */
public enum SenseKey {
	/**
	 * Indicates that there is no specific sense key information to be reported. This may occur
	 * for a successful command or for a command that receives CHECK CONDITION status because one
	 * of the FILEMARK, EOM, or ILI bits is set to one.
	 */
	NO_SENSE(0x00),
	/**
	 * Indicates that the command completed successfully, with some recovery
	 * action performed by the device server. Details may be determined by examining the additional sense
	 * bytes and the INFORMATION field. When multiple recovered errors occur during one command, the
	 * choice of which error to report (e.g., first, last, most severe) is vendor specific.
	 */
	RECOVERED_ERROR(0x1),
	/**
	 * Indicates that the logical unit is not accessible. Operator intervention may be required
	 * to correct this condition.
	 */
	NOT_READY(0x2),
	/**
	 * Indicates that the command terminated with a non-recovered error condition that
	 * may have been caused by a flaw in the medium or an error in the recorded data. This sense key may
	 * also be returned if the device server is unable to distinguish between a flaw in the medium and a
	 * specific hardware failure (i.e., sense key 4h {@link #HARDWARE_ERROR}).
	 */
	MEDIUM_ERROR(0x3),
	/**
	 * Indicates that the device server detected a non-recoverable hardware failure
	 * (e.g., controller failure, device failure, or parity error) while performing the command or during
	 * a self test.
	 */
	HARDWARE_ERROR(0x4),
	/**
	 ...
	 * 
	 * If the device server detects an invalid parameter in the CDB, it shall terminate the command
	 * without altering the medium. If the device server detects an invalid parameter in the additional
	 * parameters supplied as data, the device server may have already altered the medium.
	 */
	ILLEGAL_REQUEST(0x5),
	/**
	 * Indicates that a unit attention condition has been established (e.g., the removable
	 * medium may have been changed, a logical unit reset occurred). See SAM-3.
	 */
	UNIT_ATTENTION(0x6),
	/**
	 ...
	 * blank medium or format-defined end-of-data indication while reading or that a write-once
	 ...
	 * on the source device, the destination device, or both (see "errors detected
	 * during processing of segment descriptors").
	 */
	COPY_ABORTED(0xa),
	/**
	 ...
	 */
	/**
	 * Indicates that a buffered SCSI device has reached the end-of-partition and
	 * data may remain in the buffer that has not been written to the medium. One or more
	...
	/**
	 * Returns the {@link SenseKey} instance representing
	 * the passed <i>value</i>.
	 * @param value a sense key value
	 * @return the {@link SenseKey} instance representing
	 * the passed <i>value</i>
	 */
