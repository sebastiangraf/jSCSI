 ...
 * conceptually most important parts of its behavior can be likened to a
 * finite state machine (FSM), in which the most basic states (stages)
 * are grouped into more general states (phases). Commands send by the
 * initiator are carried out in these stages, usually without transitioning
 * to a different phase. A connection's current phase 
 * determines which stages are reachable, limiting the kind of commands the
 * initiator may issue at any given moment.
...
public final class TargetConnection implements Callable<Void> {
	...
	/**
	 * The {@link TargetSenderWorker} used by this connection for sending
	 * and receiving {@link ProtocolDataUnit}s.
	 */
	...
	/**
	 * Will manage and serve as a source of byte arrays to be used
	 * for sending Data In PDUs in the {@link ReadStage}.
	 */
	private FastByteArrayProvider dataInArrayProvider =
		new FastByteArrayProvider(4);
	
	/**
	 * <code>true</code> if and only if this connection is the
	 * first connection to be associated with its parent session.
	 * <p>
	 * This distinction is necessary because some parameters may
	 * only be declared over the leading connection.
	 */
	 ...
	 * @param isLeadingConnection <code>true</code> if and only if this
	 * connection is the first connection associated with its enclosing
	 * session
	 */
	...
	/**
	 * Returns a byte array that can be used for holding data segment
	 * data of Data In PDUs sent during the {@link ReadStage}.
	...
	/**
	 * Returns the next {@link ProtocolDataUnit} to be received on
	 * the connection.
	 * <p>
	 ...
	 * @throws InternetSCSIException if a general iSCSI protocol error
	 * has been detected
	...
	/**
	 * Serializes and sends a {@link ProtocolDataUnit} over the
	 * connection.
	 ...
	 * <p>
	 * For this method to work properly, the leading PDU send by the
	 * initiator over this connection must have been received via
	 * {@link #receivePdu()}.
	 */
	public Void call() {
		...
		try {
			...
			if (phase.execute(lastReceivedPDU)) {
				...
				if (isLeadingConnection)
					targetSession.setSessionType(
							SessionType.getSessionType(settings.getSessionType()));
	 ...
	 * <p>
	 * This method must be be called after the this connection has
	 * been added to its session.
	 */
