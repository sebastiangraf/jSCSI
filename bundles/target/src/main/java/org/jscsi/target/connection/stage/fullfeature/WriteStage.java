...
public final class WriteStage extends ReadOrWriteStage {
	...
	/**
	 * Is used for checking if the PDUs received in a Data-Out
	 * sequence actually are Data-Out PDU and if the PDUs have been received in
	 * order.
	...
	public void execute(ProtocolDataUnit pdu) throws IOException, DigestException, InterruptedException, InternetSCSIException, SettingsException {
		...
		if (scsiOpCode == ScsiOperationCode.WRITE_10)
			...
			//close connection
			throw new InternetSCSIException("wrong SCSI Operation Code " + scsiOpCode +
					" in WriteStage");
		}
		...
		if (cdb.getIllegalFieldPointers() != null) {
			 ...
			 * 
			 * Sending an error status SCSI Response PDU will not work
			 * reliably, since the initiator may not be expecting a response
			 * so soon. Also, if the WriteStage is simply left early (without
			 * closing the connection), the initiator may send additional
			 * unsolicited Data-Out PDUs, which the jSCSI Target is currently
			 * unable to ignore or process properly.
			 */
		...
		if (!initialR2T && !bhs.isFinalFlag()) {
			...
			while (!firstBurstOver && bytesReceived <= firstBurstLength) {
				...
				Target.storageModule.write(pdu.getDataSegment().array(),//source
						storageIndex + dataOutParser.getBufferOffset());//destination index
		...
		if (bytesReceived < transferLengthInBytes) {
			...
			while (bytesReceived < transferLengthInBytes) {
				...
				//send R2T
				pdu = TargetPduFactory.createReadyToTransferPdu(
						0,//logicalUnitNumber
						initiatorTaskTag,
						Target.getNextTargetTransferTag(),//targetTransferTag
						readyToTransferSequenceNumber++,
						bytesReceived,//bufferOffset
				...
				while (!solicitedDataCycleOver) {
					...
					/*
					 * Checking the final flag should be enough, but is not, when dealing with
					 * the jSCSI Initiator.
					 * This is also one of the reasons, why the contents of this while loop,
					 * though very similar to what is happening during the receiving of the
					 * unsolicited data PDU sequence, has not been put into a dedicated method.
					 */
