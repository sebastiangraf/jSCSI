 ...
 * parent class, {@link ConnectionSettingsNegotiator} instances provide
 * all methods necessary for text parameter negotiation and for creating
 ...
 *  <li>Call {@link #checkConstraints()} to check more complex requirements.
...
public final class ConnectionSettingsNegotiator extends SettingsNegotiator {
	 ...
	 * @param sessionSettingsNegotiator the {@link SessionSettingsNegotiator} maintaining all {@link Entry}
	 * objects for session-wide parameters
	 */
	 ...
	 * is negotiating and will return <code>true</code> if negotiations
	 * may proceed, or <code>false</code>, if the request for the permission
	 * to negotiate was denied.
	...
	/**
	 * This method must be called at the end of a negotiation sequence. If
	 * there was a problem during negotiations, all changes to the managed
	 * {@link Entry} objects must be discarded by setting the
	 * <i>commitChanges</i> parameter to <code>false</code>. If it is
	 * <code>true</code>, then the parameter changes will be incorporated
	 * into the updated {@link #settings}.
	 * <p>
	 * Calling this method should be ensured by a <code>try ... catch ...
	 * finally</code> block.
	 * @param commitChanges <code>true</code> if and only if the negotiated
	 * parameter changes are to be committed 
	 */
	 ...
	 * @param leadingConnection <code>true</code> if and only if the connection
	 * is the first connection of the enclosing session
	 * @param initialPdu <code>true</code> if and only if the <i>key-value</i>
	 * pairs were sent in the first {@link ProtocolDataUnit} received on this
	 * connection
	 * @param requestKeyValuePairs contains the <i>key-value</i> pairs from the
	 * initiator; processed elements will be removed
	 * @param responseKeyValuePairs will contain the <i>key-value</i> pairs from
	 * the jSCSI target
	...
	public boolean negotiate(final LoginStage loginStage,
		...
		while (keys.size() > 0) {
			...
			if (entry == null) {
				responseKeyValuePairs.add(
						TextParameter.toKeyValuePair(keys.get(0),
								TextKeyword.NOT_UNDERSTOOD));
			} else {//appropriate entry was found
				//process key=value pair and remember if there is any trouble
				everythingOkay &= entry.negotiate(loginStage, leadingConnection,
						initialPdu, keys.get(0), values.get(0),
		...
		if (initialPdu) {
			...
			everythingOkay &=
				getEntry(TextKeyword.INITIATOR_NAME).checkAccepted();
			...
			boolean normalSession = TextKeyword.NORMAL.equals(
					(String)getEntry(TextKeyword.SESSION_TYPE).value);
			if (normalSession) {
				...
				if (targetName == null ||//not declared
						!targetName.equals(Target.config.getTargetName()))//wrong name
					everythingOkay = false;
				
				//send TargetAlias
				final String targetAlias = Target.config.getTargetAlias();//might be undefined
				if (targetAlias != null) {
					responseKeyValuePairs.add(
							TextParameter.toKeyValuePair(TextKeyword.TARGET_ALIAS, 
									targetAlias));
				}
				
				//send TargetPortalGroupTag
				responseKeyValuePairs.add(
						TextParameter.toKeyValuePair(TextKeyword.TARGET_PORTAL_GROUP_TAG, 
								Integer.valueOf(Target.config.getTargetPortalGroupTag()).toString()));
			}
		}
	...
	public boolean checkConstraints() {
		...
		/*
		 * in multi-connection session, the InitiatorName of follow-up
		 * connections should be checked against the InitiatorName of
	...
	/**
	 * This method checks if any parameter changes have been committed since
	 * the last call and, if so, creates a new {@link Settings} object
	 * reflecting the current parameters.
	 * @return The current parameters
	 */
	public Settings getSettings() {
		//check if settings are up to date
		if (sessionSettingsNegotiator.getCurrentSettingsId() >
	...
	/**
	 * Updates {@link #settings} by replacing it with a more
	 * current copy of the managed parameters.
	 */
	...
	protected void initializeEntries() { 
		...
		/*
		 * Interval value (in 4-byte words) for target-to-initiator markers.
		 * The interval is measured from the end of one marker to the beginning
		 * of the next one. The offer can have only a range; the response can
		 * have only a single value (picked from the offered range) or Reject.
		 * 
		 ...
		 * can receive in any iSCSI PDU. Zero (don't care) can be used. I&T
		 * can specify (declare) the Max they can receive. This is a
		 * connection- and direction- specific parameter. The actual value
		 * used by the target will be min(This value, MaxBurstLength) for
		 * data-in and solicited data-out data.
		...
		entries.add(new NumericalEntry(
				...
				NegotiationStatus.DEFAULT,//negotiationStatus
				8192,//negotiationValue (default value, will be used if I sends 0)
		...
		/*
		 * Interval value (in 4-byte words) for initiator-to-target markers.
		 * The interval is measured from the end of one marker to the beginning
		 * of the next one. The offer can have only a range; the response can
		 * have only a single value (picked from the offered range) or Reject.
		 * 
		...
		/*
		 * This entry is not used for Settings initialization, TargetName
		 * has a session-wide scope. This entry intercepts the TargetName
		 * parameter the initiator has to declare at the beginning of
		 * normal sessions.
		 */
		entries.add(new StringEntry(
	...
	protected void updateSettingsBuilderComponent() {
		connectionSettingBuilderComponent = 
			new ConnectionSettingsBuilderComponent(entries);
	}
}
