...
public final class ConnectionSettingsNegotiator extends SettingsNegotiator {
     ...
     * 
     * @param commitChanges
     *            <code>true</code> if and only if the negotiated parameter
     ...
     * @param initialPdu
     *            <code>true</code> if and only if the <i>key-value</i> pairs
     *            were sent in the first {@link ProtocolDataUnit} received on
     *            this connection
     * @param requestKeyValuePairs
     *            contains the <i>key-value</i> pairs from the initiator;
    ...
    public boolean checkConstraints() {
         ...
         * connections should be checked against the InitiatorName of the
         * leading connection
         */
    }
     ...
     * last call and, if so, creates a new {@link Settings} object reflecting
    ...
    protected void initializeEntries() {
         ...
         * interval is measured from the end of one marker to the beginning of
         * the next one. The offer can have only a range; the response can have
         * only a single value (picked from the offered range) or Reject.
         * 
         ...
         * can receive in any iSCSI PDU. Zero (don't care) can be used. I&T can
         * specify (declare) the Max they can receive. This is a connection- and
         * direction- specific parameter. The actual value used by the target
         * will be min(This value, MaxBurstLength) for data-in and solicited
         ...
         * session-wide scope. This entry intercepts the TargetName parameter
