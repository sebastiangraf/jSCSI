...
public class TargetSession {
	...
	private static short getNextTargetSessionIdentifyingHandle() {
		short handle = nextTargetSessionIdentifyingHandle++;//no concurrency necessary
	...
	/**
	 * The {@link TargetConnection} used for exchanging messages and data
	 * with the session's initiator endpoint.
	 */
	...
	/**
	 * Keeps track of the value to expect in the <code>ExpCmdSN</code> field
	 * of the next received {@link ProtocolDataUnit}
	 */
	 ...
	 * @param connection the session's (first) {@link TargetConnection}
	 * @param initiatorSessionID the {@link ISID} specified by the initiator
	 * @param expectedCommandSequenceNumber initialization value of
	 * {@link #expectedCommandSequenceNumber}
	 * @param statusSequenceNumber the value expected by the initiator in the
	 * next {@link ProtocolDataUnit}'s <code>StatSN</code> field
	 */
	public TargetSession(final TargetConnection connection,
		...
		//initialize ConnectionSettingsNegotiator (makes sure that settings are initialized)
	...
	/**
	 * Returns the {@link SerialArithmeticNumber} representing the next
	 * expected command sequence number.
	 * <p>
	 * This value will be used both during sending (<code>ExpCmdSN</code>
	 * field) and receiving (<code>CmdSN</code>) of
	 * {@link ProtocolDataUnit}s. 
	 * @return the {@link SerialArithmeticNumber} representing the next
	 * expected command sequence number
	...
	/**
	 * Returns the {@link ISID} used by the initiator to identify this
	 * session.
	 * @return the {@link ISID} used by the initiator to identify this
	 * session
	 */
	...
	/**
	 * Returns a {@link SerialArithmeticNumber} representing the maximum
	 * command sequence number the target will accept.
	 * <p>
	 * This value will be used both during sending (<code>MaxCmdSN</code>
	 * field) and receiving (checking if PDU's <code>CmdSN</code> lies in the
	 * command sequence number window resulting from
	 * {@link #expectedCommandSequenceNumber} and {@link #commandWindowSize})
	 * of {@link ProtocolDataUnit}s. 
	 * @return the {@link SerialArithmeticNumber} representing the next
	 * expected command sequence number
	 */
	SerialArithmeticNumber getMaximumCommandSequenceNumber() {
		return new SerialArithmeticNumber(
				expectedCommandSequenceNumber.getValue() + commandWindowSize - 1);
	}
	 ...
	 * connections. If this reduces the number of connections to zero,
	 * the session will be removed from the {@link Target}'s list of active
	 * sessions.
	 ...
	 * <p>
	 * The type may and must be set just once. Repeated calls of this
	 * method will fail. <code>true</code> will be returned if the session
	 * type was set successfully, <code>false</code> if not.
