 ...
 * </ul>
 * Identification descriptors shall be assigned to the peripheral device (e.g., a disk drive)
 * and not to the currently mounted media, in the case of removable media devices. Operating
 * systems are expected to use the identification descriptors during system configuration
 * activities to determine whether alternate paths exist for the same peripheral device.
 * <p>
 * This class uses the singleton pattern since the content of the
 * DEVICE IDENTIFICATION VPD PAGE will never change.
...
public class DeviceIdentificationVpdPage implements IResponseData {
	...
	/**
	 * The joint content of the PERIPHERAL QUALIFIER and the
	 * PERIPHERAL DEVICE TYPE fields with a total length of one
	 * byte.
	 * <p>
	 * These values have the following meaning:
	 * <p>
	 * A peripheral device having the specified peripheral device
	 * type is connected to this logical unit. If the device server
	 * is unable to determine whether or not a peripheral device is
	 * connected, it also shall use this peripheral qualifier. This
	 * peripheral qualifier does not mean that the peripheral device
	 * connected to the logical unit is ready for access.
	 * <p>
	 * The Logical Unit is a direct access block device (e.g.,
	 * magnetic disk).
	 */
	...
	private DeviceIdentificationVpdPage() {
		/*
		 * For each logical unit that is not a well known logical unit, the Device
		 * Identification VPD page shall include at least one identification descriptor
		 * in which a logical unit name (see SAM-3) is indicated.
		 */
		...
		final IdentificationDescriptor identDescriptor = new IdentificationDescriptor(
				protocolIdentifier, codeSet, protocolIdentifierValid, association,
				identifierType, ScsiNameStringIdentifier.getInstance());
	...
	/**
	 * Returns the combined length of all contained IDENTIFICATION
	 * DESCRIPTORs.
	 * @return the  combined length of all contained IDENTIFICATION
	 * DESCRIPTORs
	 */
	...
	public void serialize(ByteBuffer byteBuffer, int index) {
		...
		int iddIndex = index + HEADER_LENGTH;//index of the current ident. descr.
		for (int i = 0; i < identificationDescriptors.length; ++i) {
