...
/**
 * Segment pointer sense-key-specific data is to be used, when the sense key
 * of the enclosing {@link SenseData} has the value {@link SenseKey#COPY_ABORTED}.
...
public class SegmentPointerSenseKeySpecificData extends SenseKeySpecificData {
	 ...
	 * relative to the start of the parameter list. An SD bit set to one indicates
	 * that the field pointer is relative to the start of the segment descriptor
	 * indicated by the third and fourth bytes of the 
	 ...
	 * {@link #bitPointer} field is not valid. A BPV bit set to one indicates that
	 * the {@link #bitPointer} field specifies which bit of the byte designated by
	 * the {@link #fieldPointer} field is in error. When a multiple-bit field is in
	 * error, the {@link #bitPointerValid} field shall point to the most-significant
	 * (i.e., left-most) bit of the field.
	 */
	...
	public SegmentPointerSenseKeySpecificData(
			...
			final boolean segmentDescriptor,
			final boolean bitPointerValid,
			final int bitPointer,
	...
	protected void serializeSpecificFields(final ByteBuffer byteBuffer,
		...
		byte b = byteBuffer.get(index);//SKSV bit has already been set and has to be preserved
