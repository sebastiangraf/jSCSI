...
/**
 * This is an abstract super-class for command descriptor blocks that
 * that are used for requesting information about the capacity of the
 * storage device.
...
public abstract class ReadCapacityCdb extends CommandDescriptorBlock {
	 ...
	 * <p>
	 * If the specified LBA exceeds the
	 * capacity of the medium, then the device server shall terminate the
	 ...
	 * on the last logical block after that specified in the LOGICAL BLOCK ADDRESS
	 * field before a substantial vendor specific delay in data transfer may be
	 * encountered.
	 * <p>
	 * The LOGICAL BLOCK ADDRESS field shall be set to zero if the PMI bit is set
	 * to zero. If the PMI bit is set to zero and the LOGICAL BLOCK ADDRESS field
	 * is not set to zero, then the device server shall terminate the command with
