...
public class NumericValueTest {
    ...
    public void testParseNumericValueString () {
        ...
        if (!(nd instanceof SingleNumericalValue)) fail(d + " should have been a SingleNumericValue");

        final NumericalValue ne = NumericalValue.parseNumericalValue(e);
        if (!(ne instanceof SingleNumericalValue)) fail(e + " should have been a SingleNumericValue");

        final NumericalValue nf = NumericalValue.parseNumericalValue(f);
        if (nf != null) fail(f);

        final NumericalValue ng = NumericalValue.parseNumericalValue(g);
        if (!(ng instanceof NumericalValueRange)) fail(g + " should have been a NumericValueRange");
    }
