...
public final class TaskManagementFunctionRequestParser extends
  ...
  /**
   * This enumeration defines all valid function codes, which are defined in the
   * iSCSI standard (RFC3720).
   */
  public static enum FunctionCode {
    /**
     * This is an artificial funcition code, which only used to indicate a not
     * set function code.
     */
    UNSET((byte) 0),
   ...
   * Management function codes are listed below. For a more detailed description
   * of SCSI task management, see [SAM2].
   * <p>
   * <table border="1">
   * <tr>
   * <th>Function Code</th>
   ...
   * returned as detailed in Section 10.6 Task Management Function Response. All
   * these functions apply to the referenced tasks regardless of whether they
   * are proper SCSI tasks or tagged iSCSI operations. Task management requests
   * must act on all the commands from the same session having a CmdSN lower
   * than the task management CmdSN. LOGICAL UNIT RESET, TARGET WARM RESET and
   * TARGET COLD RESET may affect commands from other sessions or commands from
   * the same session with CmdSN equal or exceeding CmdSN.
   * <p>
   * If the task management request is marked for immediate delivery, it must be
   * considered immediately for execution, but the operations involved (all or
   * part of them) may be postponed to allow the target to receive all relevant
   * tasks. According to [SAM2], for all the tasks covered by the Task
   * Management response (i.e., with CmdSN lower than the task management
   * command CmdSN) but except the Task Management response to a TASK REASSIGN,
   * additional responses MUST NOT be delivered to the SCSI layer after the Task
   ...
   * Management response but after the task management request was issued, are
   * delivered to the SCSI layer by the iSCSI layer in the initiator). The iSCSI
   * target MUST ensure that no responses for the tasks covered by a task
   * management function are delivered to the iSCSI initiator after the Task
   * Management response except for a task covered by a TASK REASSIGN.
   * <p>
   ...
   * sequence is not received (due to digest errors) for a valid TTT, the target
   * MAY treat it as a case of within-command error recovery class (see Section
   * 6.1.4.1 Recovery Within-command) if it is supporting ErrorRecoveryLevel >=
   * 1, or alternatively may drop the connection to complete the requested task
   * set function.
   * <p>
   * If an ABORT TASK is issued for a task created by an immediate command then
   * RefCmdSN MUST be that of the Task Management request itself (i.e., CmdSN
   * and RefCmdSN are equal); otherwise RefCmdSN MUST be set to the CmdSN of the
   * task to be aborted (lower than CmdSN).
   * <p>
   ...
   * the task to be aborted is allegiant at the time the Task Management Request
   * is issued. If the connection is implicitly or explicitly logged out (i.e.,
   * no other request will be issued on the failing connection and no other
   * response will be received on the failing connection), then an ABORT TASK
   ...
   * as well as abort it (i.e., the task to be aborted will not have to be
   * retried or reassigned, and its status, if issued but not acknowledged, will
   * be reissued followed by the Task Management response).
   * <p>
   ...
   * below. The TARGET WARM RESET is also subject to SCSI access controls on the
   * requesting initiator as defined in [SPC3]. When authorization fails at the
   * target, the appropriate response as described in Section 10.6 Task
   ...
   * Response (defined by [SAM2]) for this SCSI task management function may not
   * be reliably delivered to the issuing initiator port. For the TASK REASSIGN
   * function, the target should reassign the connection allegiance to this new
   * connection (and thus resume iSCSI exchanges for the task). TASK REASSIGN
   * MUST ONLY be received by the target after the connection on which the
   * command was previously executing has been successfully logged-out. The Task
   * Management response MUST be issued before the reassignment becomes
   * effective. For additional usage semantics see Section 6.2 Retry and
  ...
  /**
   * If an ABORT TASK is issued for a task created by an immediate command then
   * RefCmdSN MUST be that of the Task Management request itself (i.e.,
   ...
   * identified by the Referenced Task Tag field. Targets must use this field as
   * described in section 10.6.1 when the task identified by the Referenced Task
